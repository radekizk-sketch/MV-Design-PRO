...
describe('checkCableAmpacity — I\'dd = Idd × f1 × f2 × kg5', () => {
  it('Excel MT880: 285 × 0.9 × 1.01 × 0.82 = 212.43 A', () => {
    const result = checkCableAmpacity({
  ...
  it('Warunek niespełniony gdy Iobl > I\'dd', () => {
    const result = checkCableAmpacity({
      cable: CABLE_REFERENCE_XRUHAKXS_120,
      factors: DEFAULT_DERATING_GROUND_THREE_PHASE,
      designCurrentA: 250,
  ...
  it('Domyślne factors per ENEA Standard: 0.9 / 1.01 / 0.82', () => {
    expect(DEFAULT_DERATING_GROUND_THREE_PHASE.f1_groundResistivity).toBe(0.9);
    expect(DEFAULT_DERATING_GROUND_THREE_PHASE.f2_parallelCables).toBe(1.01);
    expect(DEFAULT_DERATING_GROUND_THREE_PHASE.kg5_parallelGroup).toBe(0.82);
  });
});
