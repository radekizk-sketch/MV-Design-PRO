    ...
    (mode: OperatingMode | RuntimeOperatingMode): string | null => {
      ...
      if (normalizedMode === 'RESULT_VIEW' && resultStatus !== 'FRESH') {
        if (resultStatus === 'NONE') {
          return 'Uruchom obliczenia, aby przeglądać wyniki.';
        }
        return 'Wyniki sa nieaktualne - wymagane jest ponowne wykonanie obliczen.';
      }
      return null;
    },
    [resultStatus],
...
export function useOverlayVisibility(): {
  ...
  if (resultStatus === 'NONE') {
    return {
      visible: false,
      hiddenReason: 'Uruchom obliczenia, aby przeglądać wyniki.',
    };
  }
  ...
  if (resultStatus === 'OUTDATED') {
    return {
      visible: false,
      hiddenReason: 'Wyniki sa nieaktualne - wymagane jest ponowne wykonanie obliczen.',
    };
  }
