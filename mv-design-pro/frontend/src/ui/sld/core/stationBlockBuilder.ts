...
export function deriveEmbeddingRole(
  ...
  // Resztkowy: 0 trunk + branch edges
    ...
    fixActions.push({
      ...
      message: `Stacja ${station.id} (${station.name}): brak krawedzi trunk i odgalezienie — nie mozna wyznaczyc roli`,
  ...
  // Safety fallback
    ...
    fixActions.push({
      code: FieldDeviceFixCodes.STATION_EMBEDDING_UNDETERMINED,
      message: `Stacja ${station.id}: nieoczekiwana kombinacja trunk=${trunkCount}, odgalezienie=${branchCount}`,
      elementId: station.id,
