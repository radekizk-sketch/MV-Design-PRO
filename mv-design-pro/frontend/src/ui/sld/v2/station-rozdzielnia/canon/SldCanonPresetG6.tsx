/**
 * Canonical SLD template — PRESET G6 (Farma wiatrowa Typ 3 / DFIG, IEC). SAME shared wind
 * skeleton as G5 (Typ 4): ONE 15 kV collector busbar (ENEA SN — 30 kV nie istnieje), a
 * connection bay to OSD with the interface protection (NC RfG), a metering bay (CT on the
 * busbar + VT/meter = granica, bez ⊟, bez PCC), and N turbine feeders with a turbine
 * transformer (0,69/15 kV Dyn). The Typ-3 difference is the SOURCE: a doubly-fed induction
 * machine — the stator ◯ sits DIRECTLY on the grid and a PARTIAL (~30%) rotor converter ~/=
 * feeds the rotor. SC is CLASSICAL (PN-EN 60909): a rotating machine behind X″ (crowbar
 * shorts the rotor at fault → cage machine), NOT a current-limited IBG — so the machine
 * share is significant and HIGHER than Typ 4, referred per-bus by the FROZEN solver.
 */
import {
  AMBER,
  AsyncMachine,
...
/** PRESET G6 — Wind Type 3 (DFIG). Collector SN + N turbine feeders (machine + partial converter). */
export function SldCanonPresetG6({ companion }: { companion: SldOzeArchetypeCompanion }): JSX.Element {
  ...
  // DFIG = a real machine: the source contribution is referred PER-BUS by the solver
  // (collector ≈ 1 kA referred through the turbine TR; terminal ≈ 10 kA local) — NOT one
  // limited IBG value. sieć/TR = the rest of the bus Ik″.
  const snMach = snSc.source_contribution.ik_contribution_ka ?? 0;
  const lvMach = lvSc.source_contribution.ik_contribution_ka ?? 0;
  ...
  const snX1 = 280;
  const snX2 = 1500;
  const title = 'SZABLON SLD — FARMA WIATROWA (Typ 3 DFIG, IEC)';
  const subtitle = `Kolektor SN ${colKv} kV · pole Liniowe · Pomiarowe | ${nTurb}× WTG ${turbMw} MW (Typ 3 · DFIG, przekształtnik częściowy) · trafo turb. ${turbTr}`;

  return (
    <g data-testid="sld-canon-g6">
      {/* ── header ── */}
      ...
      {/* ── POLE turbinowe (N feeders): Q → CB → trafo turb. → ◯ DFIG (stojan) + ~/= wirnik (~30%) ── */}
      {X.t.map((fx, i) => (
        <g key={i} data-testid={`g6-wtg-${i}`}>
          ...
          <InverterSym x={fx + 34} y={515} />
          {lbl(fx + 50, 506, '~30%', CYAN, 8.5, 700)}
          {lbl(fx + 50, 518, 'wirnik', TXT_MUTED, 8, 600)}
          {lbl(fx, 582, `${turbMw} MW · Typ 3 (DFIG)`, AMBER, 10, 800, 'middle')}
          {lbl(fx, 595, '◯ stojan + ~/= wirnik (crowbar)', TXT2, 9, 700, 'middle')}
        </g>
      ))}
      ...
      {/* ── node ② — turbine terminal 0.69 kV (TR + LOCAL machine, large) ── */}
      <NodeReadout x={X.node} y={470} n={2} title="zacisk DFIG · 0,69 kV"
        uKv={lv.un_kv} uPu={lv.u_pu} uOk={Math.abs(lv.deviation_percent) <= 5}
        ik3fMax={lvSc.max.ikss_ka} ik3fMin={lvSc.min.ikss_ka}
        ik1f="lok. (trafo Dyn turb.)" share={`TR ${fmt(lvTrShare, 1)} + maszyna ${fmt(lvMach, 1)} kA`}
