/**
 * Canonical SLD template — PRESET G7 (Farma wiatrowa Typ 1 / asynchroniczna, IEC). SAME shared
 * wind skeleton as G5 (Typ 4) / G6 (Typ 3 DFIG): ONE 15 kV collector busbar (ENEA SN — 30 kV nie
 * istnieje), a connection bay to OSD with the interface protection (NC RfG), a metering bay (CT on
 * the busbar + VT/meter = granica, bez ⊟, bez PCC), and N turbine feeders with a turbine transformer
 * (0,69/15 kV Dyn). The Typ-1 difference is the SOURCE: a SQUIRREL-CAGE induction generator wired
...
/** PRESET G7 — Wind Type 1 (squirrel-cage induction). Collector SN + N turbine feeders (machine, no converter). */
export function SldCanonPresetG7({ companion }: { companion: SldOzeArchetypeCompanion }): JSX.Element {
  ...
  // Asynchronous machine: the source contribution is referred PER-BUS by the solver (collector ≈
  // referred through the turbine TR; terminal ≈ local) — a real machine (§6.7), NOT a limited IBG.
  ...
  const snX1 = 280;
  const snX2 = 1500;
  const title = 'SZABLON SLD — FARMA WIATROWA (Typ 1, asynchroniczna, IEC)';
  const subtitle = `Kolektor SN ${colKv} kV · pole Liniowe · Pomiarowe | ${nTurb}× WTG ${turbMw} MW (Typ 1 · generator indukcyjny, bez przekształtnika) · trafo turb. ${turbTr}`;

  return (
    <g data-testid="sld-canon-g7">
      {/* ── header ── */}
      ...
      {/* ── POLE turbinowe (N feeders): Q → CB → trafo turb. → ◯G∼ (generator indukcyjny, bez ~/=) ── */}
      {X.t.map((fx, i) => (
        <g key={i} data-testid={`g7-wtg-${i}`}>
          ...
          <line x1={fx} y1={420} x2={fx} y2={512} stroke={NN_BUS} strokeWidth={2} />
      ...
      <NodeReadout x={X.node} y={478} n={2} title="zacisk async · 0,69 kV"
