...
const GREEN = '#1FA24A'; // closed / SN busbar
const CYAN = '#3BA7D6'; // nN tier
const SN_BUS = '#1FA24A';
const NN_BUS = '#3BA7D6';
const AMBER = '#FFB020';
const PINK = '#FF5FA2'; // power direction
const TXT = '#CFE9FF';
const TXT2 = '#8FA8BD';
...
// ── Canonical IEC symbols (§2) — pure presentation, no local variants ───────────
function Odlacznik({ x, y, closed = true }: { x: number; y: number; closed?: boolean }): JSX.Element {
  return <circle cx={x} cy={y} r={9} fill={closed ? GREEN : '#0A1622'} stroke={closed ? GREEN : '#FF6B6B'} strokeWidth={2} />;
}
function Wylacznik({ x, y, closed = true }: { x: number; y: number; closed?: boolean }): JSX.Element {
  return <rect x={x - 9} y={y - 9} width={18} height={18} rx={1.5} fill={closed ? GREEN : '#0A1622'} stroke={closed ? GREEN : '#FF6B6B'} strokeWidth={2} />;
}
function UziemnikIEC({ x, y, dir = 'right' }: { x: number; y: number; dir?: 'right' | 'left' }): JSX.Element {
  // odsadka + nóż ukośny + ziemia (3 malejące kreski) — LINIA, nie okrąg.
  const s = dir === 'right' ? 1 : -1;
  const gx = x + s * 22;
  return (
    <g stroke={CYAN} strokeWidth={1.6} fill="none">
      <line x1={x} y1={y} x2={gx} y2={y} />
      <line x1={gx} y1={y - 7} x2={gx + s * 9} y2={y + 7} /> {/* nóż ukośny */}
...
function TrafoDyn({ x, y }: { x: number; y: number }): JSX.Element {
  // dwa nachodzące okręgi, Δ góra (D) / Y dół (yn) — w torze (szeregowo).
  return (
    <g stroke="#E6F2FF" strokeWidth={1.6} fill="none">
...
function CtRing({ x, y }: { x: number; y: number }): JSX.Element {
  return <circle cx={x} cy={y} r={9} fill="none" stroke="#E6F2FF" strokeWidth={1.8} />;
}
function VtNoGround({ x, y }: { x: number; y: number }): JSX.Element {
  // dwa okręgi + bocznik, BEZ ziemi → woltomierz.
  return (
    <g stroke="#E6F2FF" strokeWidth={1.6} fill="none">
...
function Glowica({ x, y }: { x: number; y: number }): JSX.Element {
  // głowica kablowa = trójkąt (wierzchołek w dół).
  return <polygon points={`${x - 9},${y - 8} ${x + 9},${y - 8} ${x},${y + 9}`} fill="none" stroke="#E6F2FF" strokeWidth={1.8} />;
}
...
function InverterSym({ x, y }: { x: number; y: number }): JSX.Element {
  // kwadrat z przekątną + ~ (AC) / = (DC).
  return (
    <g stroke={AMBER} strokeWidth={1.7} fill="none">
...
/** SCADA node readout block (no frame, no table) — "wielkość = wartość" at the node. */
function NodeReadout(props: {
  ...
  return (
    <g>
      ...
      <text x={x} y={y} fill="#9FE6FF" fontFamily={SANS} fontSize={11} fontWeight={800}>{n === 1 ? '①' : '②'}</text>
      {rows.map(([k, v, c], i) => (
        <g key={k}>
...
export function SldCanonPresetG1({ companion }: { companion: SldOzeArchetypeCompanion }): JSX.Element {
  ...
  const grid = (snSc.source_contribution.ik_contribution_ka ?? 0); // faln (IBG)
  const snGridShare = Math.max(0, snSc.max.ikss_ka - grid);
  const nnTrShare = Math.max(0, nnSc.max.ikss_ka - grid);
  ...
  const preset: PresetG1 = {
    title: 'SZABLON SLD — STACJA PV 1 MW (IEC)',
    subtitle:
      'SN: pole Transformatorowe · Pomiarowe · Liniowe | TR 1000 kVA 15,75/0,8 kV Dyn5 | nN 0,8 kV układ IT · 4× falownik',
  };
  const invShare = grid / invX.length;

  return (
    <g data-testid="sld-canon-g1">
      {/* ── header ── */}
      <text x={1000} y={42} textAnchor="middle" fill="#F4F6F8" fontFamily={SANS} fontSize={22} fontWeight={800}>{preset.title}</text>
      <text x={1000} y={70} textAnchor="middle" fill={AMBER} fontFamily={SANS} fontSize={13} fontWeight={700}>{preset.subtitle}</text>
      <text x={1960} y={112} textAnchor="end" fill={CYAN} fontFamily={SANS} fontSize={11} fontWeight={700}>WYNIKI w węzłach — ze solwera (czas rzecz.) · klik węzeł → White Box</text>
      ...
      <RelayBox x={X.p1 - 72} y={310} />
      <line x1={X.p1 - 60} y1={310} x2={X.p1 - 9} y2={310} stroke={CYAN} strokeWidth={1.3} strokeDasharray="3 3" />
      {lbl(X.p1 - 95, 338, '−A14 e²TANGO-800', CYAN, 10, 700)}
      {lbl(X.p1 - 95, 351, 'I>> · I0> · G0> · U<> · f<>', TXT_MUTED, 9, 600)}
      ...
      {lbl(X.p2 - 88, 372, 'VT FD11', TXT, 11, 700)}
      {lbl(X.p2 - 88, 385, '15/√3 · bez ziemi', TXT_MUTED, 9, 600)}
      ...
      {lbl(X.p2 - 70, 428, 'POMIAR rozliczeniowy', AMBER, 11, 700)}
      {lbl(X.p2 - 70, 441, '−A16 analizator ND45 (I z AD11, U z FD11)', TXT_MUTED, 8.5, 600)}

      {/* CT AD11 — pierścień na szynie (pomiarowy, strona linii). */}
      ...
      {lbl(X.p3 - 70, 449, 'GRANICA', AMBER, 10.5, 800, 'end')}
      {lbl(X.p3 - 70, 462, 'G-ZKSN → OSD', TXT_MUTED, 9, 600, 'end')}
      ...
      {lbl(X.p3 - 110, 514, '3×XRUHAKXS 1×70/25 · L=161/191 m → SIEĆ OSD', TXT_MUTED, 9, 600)}

      {/* ── node ① readout ── */}
      <NodeReadout x={X.node1} y={228} n={1} title="szyna SN · 15,75 kV"
        uKv={sn.u_kv} uPu={sn.u_pu} uOk={Math.abs(sn.deviation_percent) <= 5}
        ik3fMax={snSc.max.ikss_ka} ik3fMin={snSc.min.ikss_ka}
        ik1f="0,12 kA" share={`sieć ${fmt(snGridShare, 1)} + faln. ${fmt(grid, 1)} kA`}
      ...
      {invX.map((fx, i) => (
        <g key={i} data-testid={`g1-inv-${i}`}>
          ...
          <PowerArrow x={fx - 14} y={642} dir="up" />
          ...
          {lbl(fx, 728, `INW ${i + 1}`, AMBER, 11, 800, 'middle')}
          {lbl(fx, 741, 'SUN2000-330KTL · 330 kW', TXT_MUTED, 8, 600, 'middle')}
          {lbl(fx, 754, 'P ≈ 0,25 MW', TXT2, 9, 700, 'middle')}
          {lbl(fx, 767, `Ik″ 3f ≈ ${fmt(invShare, 2)} kA`, TXT2, 9, 700, 'middle')}
        </g>
      ))}
      {lbl(807, 792, 'zabezpieczenia falowników: anti-islanding · U<> · f<> · df/dt', TXT_MUTED, 9.5, 600, 'middle')}

      {/* ── node ② readout ── */}
      <NodeReadout x={X.nnLbl} y={628} n={2} title="szyna nN · 0,80 kV · IT"
        uKv={nn.u_kv} uPu={nn.u_pu} uOk={Math.abs(nn.deviation_percent) <= 5}
        ik3fMax={nnSc.max.ikss_ka} ik3fMin={nnSc.min.ikss_ka}
        ik1f="≈0 (IT)" share={`TR ${fmt(nnTrShare, 1)} + faln. ${fmt(grid, 1)} kA`}
        icw={nnSc.icw_ka} icwOk={nnSc.verification.passed} />

      {/* ── LEGENDA — symbole IEC ── */}
      <rect x={80} y={838} width={1840} height={86} rx={8} fill="none" stroke="#13435A" strokeWidth={1} />
      ...
      {[
        ...
        return (
          <g key={String(label)}>
            ...
            {lbl(cx, cy + 26, String(label), TXT_MUTED, 9.5, 700, 'middle')}
          </g>
        );
      })}
    </g>
