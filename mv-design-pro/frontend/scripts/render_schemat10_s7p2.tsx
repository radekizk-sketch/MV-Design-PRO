/**
 * SCHEMAT-10 S7 etap 2 (V12K-137, GAP `S7_GAP_CROSSING_ZERO` §S7-P2) — dowód
 * wizualny węzła T: Rodziny A/B przecięć rozcięte stykiem końcowym z kropką
 * węzłową (§22.1 „skrzyżowanie ≠ połączenie"). Renderuje REALNĄ sieć
 * referencyjną (`sldSubstrate52s`, 53 stacje SN + GPZ) PRODUKCYJNĄ kanwą v3
 * (`SldCanvasV3`) na L0/L1/L2 do SVG, z paskiem metryk PRZED/PO (przecięcia
 * 13/24/24 → 0/0/0; piony/kompaktyzacja P1 bez regresu).
 *
 * Uruchomienie (cwd: mv-design-pro/frontend):
 *   CANON_OUT=<dir> npx vite-node scripts/render_schemat10_s7p2.tsx
...
const WIDTH = 1800;
const HEIGHT = 1100;
// Baza przecięć sn×sn PRZED S7 etap 2 (pomiar P-1 audytu D3 / diag S7-P2).
const CROSSINGS_BEFORE: Record<number, number> = { 0: 13, 1: 24, 2: 24 };
...
for (const lod of [0, 1, 2] as SceneLod[]) {
  ...
  const line1 =
    `Widok: ${SCENE_LOD_LABELS_PL[lod]} (L${lod}) · S7 etap 2: wezel T — przeciecia do zera (V12K-137)`;
  const line2 =
    `przeciecia ${CROSSINGS_BEFORE[lod]}→${m.crossingCount} kropki_T=${dots} ` +
  ...
  writeFileSync(`${OUT}/s7p2-l${lod}.svg`, svg);
  console.log('wrote', `${OUT}/s7p2-l${lod}.svg`, `· stacje=${scene.meta.stationCount} kropki_T=${dots}`);
}
