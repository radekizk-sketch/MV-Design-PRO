/**
 * SCHEMAT-10 S6 (V12K-137) — dowód wizualny „metryki layoutu (funkcja kosztu +
 * wykorzystanie arkusza)". Renderuje REALNĄ sieć referencyjną (fixtura
 * `sldSubstrate52s`, 53 stacje SN + GPZ) przez PRODUKCYJNĄ kanwę v3
 * (`SldCanvasV3`) na L0/L1/L2 do SVG, z paskiem KOMPLETU metryk (§3/§6) w
 * stopce — dowód, że pomiar pochodzi z tej samej geometrii, którą widzi
 * użytkownik. Geometria S6 = bajt-identyczna z gałęzią (S6 dostarcza APARAT
 * pomiarowy; kompaktyzacja = S7).
 *
 * Uruchomienie (cwd: mv-design-pro/frontend):
 *   CANON_OUT=<dir> npx vite-node scripts/render_schemat10_s6.tsx
...
for (const lod of [0, 1, 2] as SceneLod[]) {
  ...
  const line1 =
    `Widok: ${SCENE_LOD_LABELS_PL[lod]} (L${lod}) · S6: metryki layoutu (V12K-137)`;
  const line2 =
    `piony=${m.verticalLength} poziomy=${m.horizontalLength} zalamania=${m.bendCount} ` +
    `bbox=${m.contentBBoxWidth}x${m.contentBBoxHeight} inkDensity=${m.inkDensity.toFixed(4)} ` +
    `swiatlo=${m.minimumClearance} kolizje=${m.labelCollisionCount} M02=${m.subtreeIntersectionCount} ` +
    `nieortog=${m.nonOrthogonalSegmentCount} niejedn=${m.ambiguousConnectionCount}`;
  ...
  writeFileSync(`${OUT}/s6-l${lod}.svg`, svg);
  console.log('wrote', `${OUT}/s6-l${lod}.svg`, `· stacje=${scene.meta.stationCount}`);
}
