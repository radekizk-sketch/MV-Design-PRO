...
| `generator.voltage_control_not_permitted` | GENERATORS | 2 | BLOCKER | Profil NC RfG operatora nie dopuszcza trybu „Regulacja napięcia (U = const)” — zmień tryb regulacji albo profil operatora | panel: `inspector`, tab: `parametry`, focus: `control_mode` |
...
| `generator.voltage_setpoint_missing` | GENERATORS | 2 | BLOCKER | Generator w trybie regulacji napięcia nie ma kompletnej nastawy — wymagana „Nastawa napięcia U” w paśmie [0,9; 1,1] pu oraz granice mocy biernej „Q min (pobór, podwzbudzenie)” < „Q max (oddawanie, nadwzbudzenie)” | panel: `inspector`, tab: `parametry`, focus: `u_set_pu` |
