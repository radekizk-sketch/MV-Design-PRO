...
**Stan po CV-4.3 K3 (2026-09-05) — ZREALIZOWANE dla backendu.** Jądro algorytmów w `network_model/core/topologia.py` (leaf bez zależności od ENM i IR: `skladowe_spojne`, `UniaWezlow`/`scal_wezly` — port 1:1 union-find z `ybus.py`, reprezentant = najmniejszy element klasy, `polaczone`, `ma_cykl`, `przeglad_wszerz`/`przeglad_wszerz_od` z rodzicem i wspólnym zbiorem odwiedzonych, `poziomy`, `sciezka_do`); serwis ENM `enm/topology.py::derive(snapshot) -> TopologyView` (szyny, `wezel_topologiczny` CN→TN przez ZAMKNIĘTE łączniki, `wyspy` z `Wyspa.zrodla_sieciowe/maszyny/generatory` i predykatami `zasilona`/`ma_odniesienie`, `sekcje` bez transformatorów, `laczniki_otwarte`, `galezie_otwarte`, `krawedzie_pominiete`; semantyka „w ruchu" = `enm/mapping.py`). Konsumenci przepięci na jądro (pomiar grepem, zero własnych przeglądów poza jądrem w `backend/src` poza dwoma plikami `application/reference_networks/**` kasowanymi w K2 i rdzeniem solvera FROZEN — allowlista z powodem B-01): `network_model/core/graph.py::find_islands/is_connected`, `network_model/core/ybus.py::_build_merged_node_map` (macierz Y bit w bit — parytet 264 wpisów assemblera bez zmian), `enm/validator.py` E003 i E060 (`derive`), `enm/topology_ops.py` (cykle, kręgosłup), `enm/topology.py::_identify_trunk`, `application/analyses/lv_domain/{energization,graph_view}.py`, `application/analyses/fault_loop/route.py`, `application/analyses/protection/czas_wylaczenia_galezi.py`, `analysis/voltage_profile/segment_decomposition.py`, `application/sld/layout.py`. Guard `scripts/topology_single_impl_guard.py` (krok P0): `deque`/`kolejka.pop(0)`/`nx.connected_components|is_connected|shortest_path|has_path|bfs_*|dfs_*`/union-find poza jądrem = czerwony, zapadka w obie strony (pomiar: 2 pliki `reference_networks`). Parytet: `tests/enm/test_topology_view.py` — wyspy == `NetworkGraph.find_islands` (IR), węzły topologiczne == scalanie macierzy Y, E003 == walidator, wyspy/sekcje == `networkx` bezpośrednio, dla całego rejestru sieci. **Pomiar A3-05 przy tej karcie:** 4 sieci rejestru (G04/G05 `scenariusz-stAB` ×2) tor kanoniczny ODMAWIA (`NetworkGraph._validate_single_slack`: 2 źródła sieciowe) — w 2 z nich źródła leżą w OSOBNYCH wyspach (rozwiązanie per wyspa w wykonawcy, bez zmiany solvera), w 2 w JEDNEJ wyspie (dwa węzły o zadanym napięciu w jednej wyspie = zmiana rdzenia NR → B-01/OD-7). Klient (`enmToSldAdapter`, `SupplyPathHighlighter`, `sld/v3/electrical`, `engine/sld-layout`) nadal liczy własną topologię — program SLD (B-02).

**Stan po CV-4.3 K3b (2026-09-06) — A3-05 ZREALIZOWANE.** Partycja solvera „slack per wyspa" jest funkcją `TopologyView` w assemblerze (`enm/assembler.py::_wyspy_zasilone`): wyspa z jednym źródłem sieciowym → własne wejście solvera, wyspa bez źródła → węzły nierozwiązane (`None` + `non_finite_fields`), wyspa z ≥ 2 źródłami → odmowa nazwana `source.multiple_grid_sources_in_island` (ten sam predykat w bramce gotowości). Wykonawca rozwiązuje wyspy po kolei (podgrafy indukowane, te same obiekty elementów) i scala (`enm/rozplyw_wysp.py`). Kwalifikacja topologiczna wierszy zwarcia (CI-PARYTET-5) przeniesiona do assemblera (`wezly_bez_impedancji_do_odniesienia`) i domknięta: wyspa pływająca nie wchodzi do grafu solvera zwarciowego (do K3b osobliwa macierz wywracała cały bieg — 8 wpisów złotych — albo oddawała szum zerowany po fakcie). IR: `NetworkGraph.podgraf(wezly)`, `get_slack_node_ids()`. Projekcja nN: kotwice SN dwóch niezależnych systemów (scenariusze 05/06) dostają równoważnik Thevenina zamiast „brak danych: upstream_network_topology_invalid" (fikstury FE zregenerowane sankcjonowanym eksporterem). Do ostatniego klika: ekran zbieżności ui2 (`ui2/wyniki/zbieznosc`) czyta addytywne `power_flow_trace.wyspy` — tabela wysp zasilonych (szyna bilansująca, źródło, szyny PQ/PV, iteracje, zbieżność per wyspa) i wszystkie szyny bilansujące w założeniach; przy jednej wyspie ekran bez zmian. Klient SLD nadal liczy własną topologię — program SLD (B-02).

### C.2.3 Jeden assembler (§12) — inwentarz i los ścieżek (A3 §2.1–§2.2)
| Ścieżka | Miejsce | Los |
|---|---|---|
| P1 `canonical_analysis._execute_power_flow` + `map_enm_to_network_graph` | `enm/canonical_analysis.py:1642`, `enm/mapping.py` | **KANON** — jedyny assembler; prywatne `_execute_power_flow` importowane przez 8 modułów → publiczne `orchestrator.run(plan)`; `pv_bus_ids=[]` zawsze → PV z modelu (A3-04); wiele slacków → slack per wyspa z `TopologyView` (A3-05) **CV-4.1 (2026-09-05): złożenie wejścia WYCIĘTE 1:1 do `enm/assembler.py::zloz_wejscie_rozplywu(snapshot, options, graph=None) -> WejscieRozplywu` (wykonawca tylko rozwiązuje i montuje wynik; assembler nie importuje `CanonicalRun`); parytet 252 złotych hashy bit w bit; węzły nierozwiązane → `None` + `non_finite_fields` (§35). **A3-04 → ZREALIZOWANE kartą CV-4.1b (2026-09-05):** generator w trybie `meta.control_mode="REGULACJA_NAPIECIA"` (`meta.u_set_pu`, granice `q_min_mvar`/`q_max_mvar`) czyni swoją szynę węzłem `NodeType.PV` w `enm/mapping.py` (odmowa: dwa generatory regulujące na jednej szynie, szyna SLACK jako PV, węzeł PV z dodatkową regulacją falownika innego generatora na tej samej szynie); `enm/assembler.py::zloz_wejscie_rozplywu` buduje `PVSpec` z granic Q z migawki (PQSpec nie budowany dla węzłów PV); walidator ENM blokuje (`generators.voltage_control_incomplete`, BLOCKER, zmostkowany do kanonu jako `generator.voltage_setpoint_missing`) nastawę niekompletną/niefizyczną PRZED uruchomieniem rozpływu; `power_flow_trace.pv_bus_ids` przestał być zawsze pustą listą. Pomiar na rejestrze sieci referencyjnych (`tests/golden/registry.py`, 17 wpisów): PRZED 0/17 wpisów miało zmaterializowaną sieć z generatorem w regulacji napięcia (wpis G06 istniał, ale `budowniczowie=()`); PO 1/17 (G06, 2 warianty: nastawa osiągalna / nasycenie Q). Złote hashe parytetu assemblera (widok tolerancyjny `porownaj_wpis`, baza `40e49c22`): 252→264 wpisów (+12, wyłącznie G06 × {PF, SC_1F/2F/2FG/3F_max, SC_3F_min}), 0 rozbieżności ponad tolerancję w 252 wspólnych wpisach, 0 usuniętych — w tym sieci G00/G04/G05 z generatorem BESS (pole `q_min_mvar`/`q_max_mvar`/`u_set_pu` dodane w `meta` jako `None`, znalezisko KLASA NIE INSTANCJA przy okazji, szczegóły `CONVERGENCE_ROADMAP.md` CV-4.1b) bez rozbieżności — jedyny GENEROWANY artefakt wrażliwy na te pola (`sldNetwork53.ts`, hash migawki wprost, nie wynik solvera) zregenerowany sankcjonowanym generatorem. Dowód semantyczny niezależną wyrocznią: `tests/application/reference_networks/test_pandapower_cross_validation.py::TestPvNodeCrossValidatePandapower` (pandapower `create_gen(vm_pu=...)`, ten sam kontrakt matematyczny węzła PV; marker `pandapower`, nieuruchamialny w tym venv — zweryfikowany niezależnie stroną kanoniczną: moduł U węzła PV = 1,02 pu, brak nasycenia, Q≈5,7 Mvar).** **A3-05 → ZREALIZOWANE kartą CV-4.3 K3b (2026-09-06):** IR przyjmuje węzeł SLACK per źródło sieciowe (inwariant „≤ 1 SLACK w grafie" zdjęty z `NetworkGraph.add_node` — solvery FROZEN nie czytają `NodeType.SLACK`; `get_slack_node()` ścisłe); assembler buduje z `TopologyView` jedno `PowerFlowInput` na wyspę zasiloną (`WejscieRozplywu.wyspy`, `WyspaRozplywu`; podgraf wyspy przy kilku wyspach — regulator zaczepowy cudzej wyspy nie istnieje w wejściu), wykonawca scala rozwiązania (`enm/rozplyw_wysp.py`: unie węzłów/gałęzi, sumy strat i mocy szyn bilansujących; addytywne `raw_result.wyspy`/`power_flow_trace.wyspy` tylko przy ≥ 2 wyspach); dwa źródła w JEDNEJ wyspie = odmowa NAZWANA `source.multiple_grid_sources_in_island` (assembler + bramka gotowości `_check_power_flow`, jedna `TopologyView`; polityka rozdziału mocy → OD-7). Zwarcie: wyspy bez impedancji do odniesienia poza `solve_graph` (Y-bus nieosobliwa; wiersz bez solvera), dwa GPZ w jednej wyspie liczone superpozycją Y_Q. Złote hashe 264: 30 odmów → wyniki, 2 odmowy nazwane, 42 wpisy bez zmiany liczb węzłów zasilanych, 190 bit w bit; P11 132: 12 z odmowy na wynik/odmowę nazwaną. Wyrocznia pandapower (`tests/golden/wyrocznie/pandapower.py`, jeden most ENM → pandapower): rozpływ 02/05 |ΔU| ≤ 5,4·10⁻⁵ pu, zwarcie 3F 02/05/06 |ΔIk''|/Ik'' ≤ 10⁻⁴. |
