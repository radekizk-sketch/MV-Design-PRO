...
| R2 | legacy `analysis_runs` (+`analysis_runs_index`) | `AnalysisRunService`, porównania PF/zabezpieczeń | druga prawda biegów | **CZĘŚCIOWO ZREALIZOWANE (CV-3.3-A, 2026-09-05)**: podmoduły martwe (`export_service.py`, `results_inspector.py`, `orchestrator.py`, `catalog_context.py`, `dtos.py` — zero konsumentów poza własnymi testami) **DELETE** wraz z testami; `analysis_run/__init__.py` eksportuje wyłącznie żywe (`AnalysisRunService`, `read_model`). Tabela żywa (`AnalysisRunService`, `result_invalidator.py`, `read_model.py`, repozytoria, `power_flow_comparison`/`protection_analysis`/`comparison` czytające ją bezpośrednio) pozostaje — przepięcie na R1: karta **CV-3.3-B** |
