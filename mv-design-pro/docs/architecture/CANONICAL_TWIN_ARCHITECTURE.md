...
| R4 | in-memory `_runs` (V12.6), `_batches`, `_coordination_results`, `_interpretation_cache`, `_overrides_store`, `_config_store` | A9 §3.3 poz. 10 | giną z procesem — nieodtwarzalne | **CONSOLIDATE**: biegi → R1 (`_runs`/V12.6 — poza zakresem CV-3.3-B, nienazwany w jej §0); serie → tabela `run_batches` (`_batches` — **PREAUTORYZOWANY PODZIAŁ do karty CV-3.3-C** przy CV-3.3-B: inwentarz w `CONVERGENCE_ROADMAP.md` wiersz CV-3.3-C, rozmiar porównywalny z B1–B3 tej karty — nowa tabela SQL + ORM + repozytorium + przepięcie `BatchExecutionService` + ~685 linii istniejących testów do przepisania, nie „później" bez inwentarza); nadpisania SLD/konfiguracje → magazyn projektu (poza zakresem CV-3.3-B) |
...
| E4 | `BatchExecutionService` (`_batches` in-memory) | produkcja (`batchStore.ts`) | **PROMOTE** do orkiestratora (`ExecutionPlan` + `ExecutionBackend`), trwały rejestr serii. **PODZIELONE do karty CV-3.3-C** (preautoryzowany podział przy CV-3.3-B, 2026-09-05 — rozmiar porównywalny z B1–B3 tamtej karty): singleton procesowy `_batch_service` (`api/batch_execution.py`) trzyma `BatchJob` WYŁĄCZNIE w pamięci (`_batches`/`_case_batches`/`_pinned_hashes` — 3 słowniki), ginie z procesem; konsument FE (`frontend/src/ui/study-cases/batchStore.ts`) mapuje 1:1 na 4 końcówki REST bez wiedzy o mechanizmie trwałości backendu — repointing nie wymaga zmian frontendu, JEŚLI kontrakt HTTP zostanie zachowany bit w bit |
