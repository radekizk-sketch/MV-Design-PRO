...
| C4 | P23 `Study/Scenario/Run/Snapshot` (`switches_state_ref: Any`…) | `application/study_scenario/models.py`, `orchestration.py` | brak API; **STOP (CV-3.2, 58e520ce)** — ponowna weryfikacja grepem znalazła konsumenta produkcyjnego POZA `builder.py`: `analysis/reporting/pdf/p24_plus_report.py:45` importuje `ScenarioComparisonEntry`/`ScenarioComparisonView` z `analysis/scenario_comparison/models.py` i realnie je konsumuje (parametr `scenario_comparison`, sekcja P27 raportu); import tego modułu URUCHAMIA `analysis/scenario_comparison/__init__.py` → `builder.py` → `application.study_scenario.models` (transytywna zależność). Kasacja C4 usunęłaby ten łańcuch — karta §0.3 nakazuje STOP zamiast przepięcia bez decyzji. `export_p24_plus_report_pdf` sam nie ma żadnego wołającego poza własnymi testami (`test_pdf_report_p24_plus.py`, `test_koperta_kontekstu.py`) — więc P24+ jest RÓWNIEŻ niewpięty w ścieżkę użytkownika, ale to ODDZIELNY byt od C4, poza inwentarzem tej karty | **NIE USUNIĘTO** — czeka na decyzję właściciela: (a) wpiąć P24+ w realną ścieżkę i zbudować `ScenarioComparisonBuilder` naprawdę, (b) usunąć sekcję P27 z P24+ razem z C4, albo (c) uznać P24+ za osobny dług i skasować go osobną kartą przed C4 |
