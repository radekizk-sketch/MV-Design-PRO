...
| **E. Odbiory i generacja** | odbiory, silniki, popyt zagregowany, PV, wiatr, generacja synchroniczna/asynchroniczna, BESS, ładowanie EV, hybrydy, DER sterowalne | dekompozycja DER, encja silnika: WSPIERA TERAZ; stan BESS (SOC, energia, dyspozycja w czasie): NIE WSPIERA — `bess_mode`/`soc_min_percent`/`soc_max_percent` zapisywane bez czytelnika w solverach i analizach (korekta 2026-09-09, `../plan/MAPA_DOMKNIECIA_PRODUKTU_2026-09.md` §3a.I) → W6; wdrożenie od G01 |
...
| **V. White Box / forensic** | INPUT → SOURCE → ASSUMPTIONS → REVISION ENVELOPE → TOPOLOGY → COMPUTATIONAL INPUT → INTERMEDIATE → EQUATIONS → RESULT → LIMITS → PASS/WARN/FAIL → PROVENANCE; zielona wartość w UI bez dowodu nie wystarcza; testy nie są dowodem fizyki | inwariant produktu; WDRAŻAMY (wyrocznie rejestru); `trace_v2` skasowany 2026-09-10 (ślad kanoniczny = inline + proof engine, nie druga ścieżka) |
...
| Dynamika: RMS, stabilność, odpowiedź częstotliwościowa, rozruch silników, grid-forming/following, LVRT/HVRT | `FABRICATED`/`PARTIAL` (korekta 2026-09-16: tor produkcyjny = ocena progowa z opcji biegu prezentowana jako `reportable`; `stability_rms` bez sprzężenia z siecią i bez konsumentów; T14/T15 tautologia w ewaluatorze — `../plan/SYNTEZA_DOMKNIECIA_PRODUKTU_2026-09.md` §0/§2; rozruch na DTO bez encji silnika — A11-09) → W6-0…W6-8 | odbiór silnikowy jako encja; parametry dynamiczne DER w katalogu; nie blokować: model musi nieść stałe czasowe i tryby |
...
| White Box / forensic: dane źródłowe → proweniencja → założenia → envelope → migawka → topologia → wejście solvera → wartości pośrednie → wynik → walidacja → interpretacja | `PARTIAL` (`CanonicalRun` zamraża migawkę; ślad inline; `trace_v2` skasowany 2026-09-10 (A3-12); pola proweniencji stałe — A2 §1.5) | pełna droga wyniku, żaden test snapshotowy nie jest wyrocznią fizyki (§23, §32) |
