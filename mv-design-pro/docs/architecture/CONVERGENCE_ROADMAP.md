...
| CV-2.1 | `ModelRevision` PRZY MAGAZYNIE ENM (magazyn modelu jest plikowy, nie SQL — rewizja nie może być „w tej samej transakcji” z tabelą): niezmienny plik migawki per rewizja `<digest>.rev/<n>.json.gz` (gzip, kanoniczny JSON) + wpis dziennika rozszerzony o `hash_sha256`, `parent` i PEŁNY ładunek komendy; kolejność zapisu: plik rewizji → HEAD → zatwierdzenie dziennika; sierota (plik bez wpisu) usuwana przy ładowaniu; `LIMIT_WPISOW` dziennika usunięty (dziennik = kompletny rejestr rewizji); `checkout(klucz, n)`; szczegóły: karta CV-2 (rozstrzygnięcia R1–R8, OW-8/OD-4) | `checkout(n).hash == revision[n].hash` dla całego rejestru sieci wzorcowych; dziennik zmian = projekcja rewizji (parity z dzisiejszym API dziennika) | bieg bez envelope = czerwony |
