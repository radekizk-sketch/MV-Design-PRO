...
| # | Luka | Dowód |
|---|---|---|
| L1 | **PRZEFORMUŁOWANA — to NIE jest brak magazynu, tylko TRZY niedokończone magazyny** | KOREKTA po bramce końcowej (2026-09-08). Napisałem „brak trwałego magazynu placement/route" — **to nieprawda**. MV ma: **(1)** trwały magazyn ORM — `sld_node_symbols` (`node_id`, **`x`**, **`y`**, `label`, `is_connection_node`) i `sld_branch_symbols` (`branch_id`, `from_node_id`, `to_node_id`, **`points_jsonb`** = wierzchołki trasy), `models.py:663-686`, z `SldRepository` wpiętym w `UnitOfWork` i konsumentami w `network_wizard/service.py` oraz archiwum projektu; **(2)** `domain/geometry_overrides.py` (303 linie: `MOVE_DELTA`/`REORDER_FIELD`/`MOVE_LABEL`, zakresy NODE/BLOCK/FIELD/LABEL/EDGE_CHANNEL, kanonizacja, hasz, walidacja zerwanych referencji) z **zamontowanym routerem** `api/sld_overrides.py` (`main.py:187`) — ale trzymanym w **słowniku w pamięci** (`_overrides_store: dict = {}`, komentarz „In production this would use a repository/DB"), więc **znika przy restarcie**; **(3)** typy `GeometryOverride*` zduplikowane we froncie (`ui/workspace/types.ts:377` i `ui/contracts/shared.ts:220`) — **zero konsumentów**. Realna luka: **fragmentacja i brak wpięcia w scenę v3** (`buildScene` liczy własną geometrię i ignoruje zapisane współrzędne), a nie brak magazynu. |
