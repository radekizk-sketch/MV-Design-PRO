...
| [`screenshots/sld-with-gpz-loaded.png`](screenshots/sld-with-gpz-loaded.png) | `#sld` z aktywnym GPZ + odcinkiem + stacją | **OPERACYJNY SCADA SLD V2 — pełny tor mocy** zgodny z goal §6: GPZ-Demo → pole SN → odcinek 1.5 km → **Stacja A — Demo (mini-RMU)**. Pokazuje: szyna 110 kV, **TR1 + TR2** (transformatory 110/SN) z portami SN, **2 sekcje szyn SN** (Sekcja 1 + Sekcja 2), 6 pól liniowych, czerwony tor do stacji na dole, sprzęgło sekcyjne 1-2. Drzewo topologii po lewej: GPZ-Demo, szyny S1/S2/110 kV TR1/TR2, zaciski techniczne SN 1-3 (porty), Stacja A. Top bar SCADA: BUDOWA aktywna, Blokery **1**, Gotowość **88%**, Elementy **21**, Pola SN **10**, **Stacje SN/nN 1**, Długość SN **1.50 km**, Transformatory **3**. Status „Gotowość obliczeń: zablokowane" + 3 blokery (np. „Łącznik 'Wyłącznik pola 1' nie ma przypisania" — brak aparatu) + 4 ostrzeżenia. Polski UI, dark SCADA tło, NIE 0.00. |
| [`screenshots/sld-gpz-with-station.png`](screenshots/sld-gpz-with-station.png) | Snapshot tego samego widoku | Kopia stanu po `insert_station_on_segment_sn` z TR 400 kVA 15/0.4 kV, polami IN/OUT/TR. |
