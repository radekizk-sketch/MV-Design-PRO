...
| 28 | Labels not on wires | PARTIAL | prior redesign moved labels off pills; declutter exists. Render-verified fix (2026-07): cable segment/run labels used a Tailwind class (`text-[11px]`) with no font-family → default 16px SERIF in CSS-less contexts (SVG export, standalone), oversized + colliding + breaking the declutter's ~11px calibration. Now explicit `fontSize=11`/`fontFamily=sans-serif` (matches siblings + declutter). Remaining (render-proven): dense trunk-band + station name/kVA overlap at lod2 = station-pitch/declutter-coverage issue (next increment) |
