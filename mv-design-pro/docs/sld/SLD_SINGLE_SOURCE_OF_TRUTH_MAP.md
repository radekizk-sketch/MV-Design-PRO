...
| Aspekt | Single Source of Truth | Sciezka |
|--------|------------------------|---------|
| **Mapowanie ElementType → benchmarkSymbolId** | `SymbolResolver.ts` | `frontend/src/ui/sld/SymbolResolver.ts` |
...
- Style benchmark: sldCanonicalStyle.ts jest jedynym zrodlem (voltageColors.ts oznaczony jako legacy — prefer sldbenchmarkStyle)

### 3.3 Legacy do monitorowania

| Element | Status | Uwagi |
|---------|--------|-------|
| `voltageColors.ts` | Legacy (prefer sldCanonicalStyle.ts) | Nie usuwac jeszcze — moze byc w uzyciu |
