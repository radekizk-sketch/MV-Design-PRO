...
| C-9 | **[ZAMKNIĘTE 2026-08-06, karta S9-7/8; ZWERYFIKOWANE 2026-08-07, karta S9-12. Człon napięcia na etykiecie przęsła niesie oznacznik `Un=` („S08 ↔ S09 · YAKXS 3×120/16 · **Un=20 kV** · l = 50 m" — `layout/lineLabel.ts` `formatRatedVoltageKv`, przypięte `lineLabel.test.ts`); pomiar S9-12: 46 etykiet z `Un=` na fixturze referencyjnej L2, zero gołych „N kV" na przęsłach. Wybór formatu ROZSTRZYGNIĘTY DANYMI (weryfikacja katalogu S9-12): katalog kabli (`backend/.../catalog/types.py` `CableType`, `mv_cable_line_catalog.py`) niesie JEDNĄ wartość `voltage_rating_kv` (np. 20,0) i NIE niesie U₀ — para „U₀/U 12/20 kV" z audytowej propozycji byłaby fabrykacją danej, której model nie ma; `Un=` jednoznacznie znakuje daną katalogową izolacji względem napięcia pracy sekcji („15 kV") bez zgadywania]** **PODNIESIONE PONOWNIE I ZAMKNIĘTE OSTATECZNIE (2026-08-08, przegląd nadzorcy + decyzja właściciela).** Zastrzeżenie nadzorcy: `Un` jest symbolem napięcia znamionowego SIECI, więc oznacznik wybrany do ODRÓŻNIENIA danej kabla nosi symbol tego, od czego ma ją odróżniać — na jednym rysunku stoją „Sekcja 1 · 15 kV" i „… · Un=20 kV" (obejrzane na produkcyjnej kanwie, zrzut `ocena-gpz`). Zaproponowano `Ur=` (wielkość znamionowa URZĄDZENIA wg IEC, ta sama liczba glifów, więc zero kosztu szerokości arkusza i zero zmian w pinach geometrii). **DECYZJA WŁAŚCICIELA: ZOSTAJE `Un=`** — w praktyce rysunkowej projektanta zapis czyta się jednoznacznie. Wiersz zamknięty; nie otwierać po raz trzeci bez nowej przesłanki. **Napięcie znamionowe kabla podane obok napięcia pracy bez rozróżnienia.** Opis odcinka brzmi „S08 ↔ S09 · YAKXS 3×120/16 · **20 kV** · l = 50 m", a sekcja obok „Sekcja 1 · **15 kV**". Czytelnik nie wie, czy to niezgodność modelu, czy dane katalogowe kabla. | `czytelnosc-srednia-L2-ciemny.png`; zrzuty `s9-12-pokazowa-L2-*.png` | ZW, PSN | 2 | Napięcie znamionowe izolacji kabla oznaczyć jednoznacznie (np. `U₀/U 12/20 kV`) albo usunąć z opisu przebiegu. |
| C-10 | **[OBSERWACJA POZYTYWNA — nic do zamknięcia]** **Zgodność symboli z IEC 60617 na poziomie L2 jest dobra** (obserwacja pozytywna): transformator dwuuzwojeniowy jako dwa okręgi, odłącznik `Q1`, uziemnik `QE1`, ogranicznik przepięć jako trójkąt, generator jako okrąg `G`, szyna jako gruba kreska; role pól opisane po polsku („pole transformatorowe", „pole pomiarowe", „pole liniowe"). **(opinia)** rysunek na L2 wygląda jak rysunek wykonawczy. | `czytelnosc-srednia-L2-ciemny.png` | CAD | — | — |
| C-11 | **[OBSERWACJA POZYTYWNA — nic do zamknięcia]** **Declutter działa poprawnie** (obserwacja pozytywna): liczba nachodzących na siebie prostokątów etykiet wynosi **0** na wszystkich sześciu kombinacjach sieć × poziom; system uczciwie melduje „Ukryto N opisów — przybliż, aby zobaczyć". | pomiar `kolizjeEtykiet` = 0 (6/6) | CAD | — | — |
| C-12 | **[OBSERWACJA POZYTYWNA — nic do zamknięcia]** **Motyw jasny na kanwie jest poprawny** (obserwacja pozytywna): tło `rgb(255,255,255)`, rysunek ciemnozielony — czytelny jak wydruk CAD; motyw ciemny `rgb(11,15,20)`. Oba motywy potwierdzone pomiarem `background-color` kanwy po realnym przełączeniu. | `czytelnosc-srednia-L2-jasny.png` vs `czytelnosc-srednia-L2-ciemny.png` | CAD | — | — |
...
| C-14 | **[ZAMKNIĘTE 2026-08-07, karta PROPORCJE — rejestr `PROPORCJE`]** **Napis dominuje symbol, który opisuje.** Zgłoszenie właściciela 2026-08-07 („brak proporcji, grubości", zrzut `b2-po-ciemny.png`): „wysokość napisu «Q1» ≈ 3–4× wysokość symbolu odłącznika". POMIAR POTWIERDZIŁ w produkcie: mechanizm powiększania awaryjnego (KD-11 `minReadableFontSize`) przypina napis do 9 px ekranu, a rysunek kurczy się razem z kamerą — stosunek napis:symbol rośnie jak 1/skala i NIE MIAŁ ŻADNEGO SUFITU. Zmierzone na fixturze 53 stacji: oznacznik aparatu **0,33** przy pracy z bliska, **2,82** przy skali dopasowania, **7,50** przy `MIN_SCALE`; nazwa stacji wobec zwiniętego bloku **3,75** przy `MIN_SCALE`. Hipoteza „im głębszy zoom, tym gorzej" POMIAREM OBALONA — przy zoomie do środka stosunek jest STAŁY (0,33–0,38 od skali 1,0 do `MAX_SCALE`); proporcje pękają przy ODDALANIU. | `scripts/pomiar_proporcje.tsx` (iloczyn {klasa typografii} × {LOD} × {skala}); zrzuty `proporcje-przed-*` vs `proporcje-po-*` | CAD, PSN | **3** | Sufit powiększenia per klasa typografii, z TEGO SAMEGO źródła co dolna granica S9-7/8; napis, którego nie da się narysować proporcjonalnie, nie jest rysowany i jest policzony. |
| C-15 | **[ZAMKNIĘTE 2026-08-07, karta PROPORCJE]** **Aparatura wypadła z hierarchii grubości.** Zgłoszenie właściciela pkt 2: „tor prądowy grubszy od kreski aparatu ok. 8–10× — odwrócona hierarchia, aparat rysowany włosowo". POMIAR POTWIERDZIŁ: podłoga ekranowa S9-8 (`MIN_TRUNK_STROKE_SCREEN_PX`) objęła TORY, a glify rysowały się stałą świata `V3_STROKE_APPARATUS` bez żadnej kompensacji. Stosunek szyna:aparat NA EKRANIE — **3,33** przy skali 1,380 (proporcja projektowa), **15,70** przy kadrze „Dopasuj widok" (0,133), **41,67** przy `MIN_SCALE`; magistrala:aparat odpowiednio 2,00 / 9,42 / 25,00 — liczba „8–10×" ze zgłoszenia to dokładnie kadr dopasowania. | `scripts/pomiar_proporcje.tsx` tabela (d) | CAD, PSN | **3** | Aparat wchodzi do TEJ SAMEJ jednorodnej kompensacji co tory (wagi świata nietknięte) — proporcje stają się niezależne od skali. |
...
| P-5 | **[OBSERWACJA POZYTYWNA — nic do zamknięcia]** **Przeciąganie (pan) jest płynne** (obserwacja pozytywna): mediana 15,9 ms, **zero** klatek powyżej 33 ms na sieci 51 stacji. | pomiar `praca-pan-duza` | UX | — | — |
...
| # | Opis | Dowód | Soczewka | Waga | Naprawa (jednym zdaniem) |
|---|------|-------|----------|------|--------------------------|
| B-1 | **[OBSERWACJA POZYTYWNA — nic do zamknięcia]** **Wstawienie stacji NIE rozsypuje układu** (obserwacja pozytywna): cały ogon magistrali przesuwa się o **dokładnie tę samą** wartość `+752` w osi X przy `dy = 0`; brak przetasowań, brak ruchu w pionie, brak zmiany kolejności. Zachowanie deterministyczne i przewidywalne. | pomiar `budowa-stabilnosc-layoutu`; `budowa-stabilnosc-przed.png` / `budowa-stabilnosc-po.png` | PSN, CAD | — | — |
| B-2 | **[ZAMKNIĘTE 2026-08-07, karta B-2: kamera KOTWICZY się na obiekcie wskazanym przez operację. Pomiar na żywym backendzie (50 → 51 stacji, kanwa 1600×1000) zrewidował mechanizm znaleziska: po karcie S9-1 świat mieści się w kadrze (6 710 j.św.), więc „ogon nie wyjeżdża" — skacze KAMERA, bo każda zmiana migawki wywoływała bezwarunkowy refit. PRZED: praca na L2 → skala 1,380 → 0,153 (**9,01× oddalenia**), poziom **L2 → L0**, wstawiona stacja **79,6 × 28,6 px**; praca na L1 → 4,56×, L1 → L0. PO: skala i poziom szczegółu ZACHOWANE (oddalenie 1,00×), stacja **717,6 × 404,3 px**, odsunięcie od środka kadru 0 px. Naprawa objęła KLASĘ „kamera po operacji zmieniającej model": jedno źródło wskazania wspólne z selekcją (`ui/topology/wskazanieOperacji.ts`), kotwica per poziom szczegółu (`canvas/viewAnchor.ts`, kandydat nierozwiązywalny pomijany — `continue_trunk_segment_sn` wskazuje szynę, której rysunek nie nosi), akcja kamery `'kotwicz'` (skala nigdy nie rośnie, centrowanie w prostokącie bezpiecznym, LOD tą samą histerezą co zoom) oraz predykat „ten sam hasz migawki ⇒ kamera nietknięta" (odświeżenie `refresh_snapshot` przestało wywracać kadr). Ta sama maszyneria domknęła klasę poboczną wykrytą pomiarem: 18 powierzchni wołało „pokaż na schemacie" (`centerSldOnElement`), a kanwa v3 nigdy tego nie czytała — teraz wskazanie kadruje element tak samo jak zapis operacji. Świadome pominięcia: `delete_element` (backend celowo nie wskazuje usuniętego elementu), `undo`/`redo` (brak wejścia w UI). Zrzuty: `audyt-2026-08/b2-{praca,przed,po}-{ciemny,jasny}.png`. Rejestr: wiersz `B-2` w `docs/v12xx/REJESTR_KONFLIKTOW.md`]** **Ogon przesuwa się o pełną szerokość stacji, więc obraz skacze.** Przesunięcie 752 j.św. przy jednokreskowym pasie (C-1) oznacza, że po wstawieniu stacji cała prawa część rysunku wyjeżdża z kadru; przy sieci 51 stacji rysunek ma już 66 tys. px, więc kamera nie nadąża za miejscem edycji. | pomiar jw. + tabela 2.1; dowód domknięcia: `frontend/scripts/{b2_zbuduj_pare.mjs,pomiar_b2_kamera.tsx}` + `canvas/__tests__/kotwicaWidoku.contract.test.tsx` | PSN, UX | 2 | Po wstawieniu utrzymać kamerę na obiekcie wstawionym (zakotwiczenie widoku na zmianie). |
| B-3 | **[OBSERWACJA POZYTYWNA — nic do zamknięcia]** **Stan pusty i kreator GPZ są dobrze poprowadzone** (obserwacja pozytywna): 1 klik do kreatora, 7 nazwanych kroków, jawne „następny krok" w pasku kontekstu. | `budowa-stan-pusty.png`, `budowa-po-kliku-wstaw-gpz.png` | UX, PSN | — | — |
| B-4 | **[ZAMKNIĘTE 2026-08-06, karta S9-5: ciag SN buduje sie z rysunku — „Wyprowadz ciag glowny SN” i „Rozpocznij odgalezienie” na zrodle GPZ i na szynie sekcji, „Zakoncz odcinek stacja SN/nN” na odcinku, „Kontynuuj ciag” / „Rozpocznij odgalezienie” na stacji; e2e: 15 stacji zbudowanych wylacznie prawym klikiem]** **Dalsza budowa nie ma ścieżki na kanwie.** Po wstawieniu GPZ na kanwie są tylko trzy przyciski układów DER (`+ PV`, `+ BESS`, `+ FW`). Operacji, które faktycznie budują sieć SN — *wstaw stację na odcinku*, *przedłuż magistralę*, *rozpocznij odgałęzienie* — nie ma ani na pasie narzędzi, ani w menu kontekstowym (P-7); wykonalne są wyłącznie przez API/kreatory poza schematem. | wypis afordancji `budowa-afordancje-pusty`; pomiary menu kontekstowego (P-7) | PSN | **3** | Dodać na kanwie operacje ciągu SN (odcinek / stacja na odcinku / odgałęzienie) obok palety DER. |
...
| W-2 | **[OBSERWACJA POZYTYWNA — nic do zamknięcia]** **Pasek pochodzenia nakładki działa** (obserwacja pozytywna): po biegu pokazuje „Moduł: zwarcie trójfazowe · Przebieg: `61603f65…` · Czas ukończenia: 6.08.2026, 07:07:18" — czyli wiązanie wynik↔rysunek istnieje, brakuje samego rysowania wartości. | pomiar jw. | ZW | — | — |
...
| W-7 | **[OBSERWACJA POZYTYWNA — nic do zamknięcia]** **Tabela wyników zwarciowych jest bardzo dobra** (obserwacja pozytywna): kolumny `Punkt zwarcia`, `Rodzaj zwarcia`, `Prąd zwarciowy początkowy Ik″ [kA]`, `Prąd udarowy ip [kA]`, `Prąd cieplny Ith [kA]`, `Moc zwarciowa Sk`, sekcja „Założenia" z `Metoda obliczeń IEC 60909`, `Współczynnik napięciowy c = 1,10`, `Czas cieplny 1,00 s`; nazwy punktów czytelne („Turbina wiatrowa 1 MW (Vestas V90)", „Stacja przemysłowa 1 MVA, 4 odpływy nN"). | `wyniki-pulapka-nawigacji.png` | ZW | — | — |
| W-8 | **[ZAMKNIĘTE 2026-08-07, karta S9-13 — dowód e2e na żywym backendzie z DWOMA biegami (`e2e/s913-porownanie-z-kanwy.spec.ts`, zarazem skrypt renderujący zrzuty). Diagnoza sondy: elementy porównawcze mieszkają w panelu filtrów doku „Warstwy" (dok domyślnie zwinięty — stąd `filter-panel false`), a tryb różnic A/B miał JEDYNE wejście przez ekran porównań (klasa KLIENT-BEZ-DOSTAWCY z V12K-326 domknięta wcześniej po stronie końcówki, ale bez wejścia z kanwy). Naprawa: sekcja „Porównanie A/B" panelu filtrów (`sld-v3-result-ab`) — wybór drugiego UKOŃCZONEGO biegu tego samego rodzaju i przypadku (runStore, bez nowych wołań), „Pokaż różnice" woła istniejącą końcówkę `POST /api/short-circuit-comparisons/sld-overlay` (A = wybrane odniesienie, B = bieg na schemacie); etykiety niosą WARTOŚĆ B obok delty (kontrakt 1.3.0, metryki `B_*` addytywnie — wartości B poza predykatem wagi, predykaty parami); stany zablokowane UCZCIWE z akcją (brak biegu ⇒ obliczenia; brak drugiego biegu tego samego rodzaju ⇒ obliczenia; rodzaj bez dostawcy nakładki, np. rozpływ ⇒ ekran porównań; OUTDATED ⇒ ta sama treść co tryb porównawczy — jedno źródło świeżości S9-11 `activeCaseResultStatus`, stale-badge bez zmian). Zrzuty: `audyt-2026-08/s9-13-roznice-L2-{ciemny,jasny}.png`, `s9-13-blokada-brak-drugiego-biegu.png`]** **Nakładka delta / porównanie A/B — stan faktyczny: nieosiągalna z kanwy.** Elementy `sld-v3-result-comparison` i `sld-v3-result-comparison-blocked` nie pojawiają się po biegu (obie treści puste), podobnie `sld-v3-result-filter-panel` (`false`) i `sld-v3-result-stale-badge` (pusty). Nie oceniam ich działania — odnotowuję, że po pojedynczym biegu nie da się ich wywołać z kanwy. | pomiar `wyniki-nakladka-poprawny-stan`; dowód domknięcia: e2e `s913-porownanie-z-kanwy` | ZW | 2 | Osobna karta — wymaga dwóch biegów i jawnego wejścia w porównanie z poziomu schematu. |
...
| E-7 | **[OBSERWACJA POZYTYWNA — nic do zamknięcia]** **Zestaw formatów jest ambitny i właściwy** (obserwacja pozytywna): obok rysunkowych (SVG/PDF/PNG/DXF) są formaty wymiany modelu — SCD (IEC 61850) i CIM (IEC 61970/61968). | `eksport-formaty-menu.png` | CAD, ZW | — | — |
