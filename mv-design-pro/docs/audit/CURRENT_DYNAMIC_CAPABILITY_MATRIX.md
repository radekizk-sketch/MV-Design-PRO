...
| Komenda regulacji (`komenda_regulacji`) w torze czasowym | `MODEL_ONLY` + `REFUSED_BY_CORE` | — | kontrakt: `enm/scenariusze.py:239-250`; odmowa: `enm/adapter_dynamiki.py` (`KOD_ZDARZENIE_NIEOBSLUGIWANE`) | `test_adapter_dynamiki.py` |
...
| Działanie zabezpieczenia jako zdarzenie dynamiczne | `NOT_IMPLEMENTED` | — | brak w `RodzajWpisu` (`zdarzenia.py:49-56`) i w `ZdarzenieDynamiczne` (`scenariusze.py:263-272`) | — |
...
| Kanał częstotliwości szyny `f(t)` | `NOT_IMPLEMENTED` | — | `silnik.py:555-605` — kanały sieciowe to WYŁĄCZNIE `u_pu@` i `kat_deg@` | — |
