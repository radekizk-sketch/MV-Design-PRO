...
| W-07 | **Akcja naprawcza wykonuje naprawę**, nie nawiguje: otwiera właściwy formularz z fokusem pola, proponuje wartość, albo stosuje auto-fix (przypisanie typu katalogowego). Nota (2026-09-05, V12K-338): pole `fix_action_id` nazwane w pierwotnym teście tego wiersza było fantomem bez wykonawcy i zostało usunięte z kontraktu — ambicja W-07 (naprawa wykonuje się sama, nie tylko nawiguje) pozostaje NIEZREALIZOWANA i wymaga osobnego mechanizmu, nie odtworzenia skasowanego pola. | test: dla każdego kodu gotowości z wymaganym auto-fix istnieje executor wykonujący realną naprawę (nie tylko nawigację); brak executora = czerwień guarda |
