...
| `TimeSeriesView` | QSTS (PF w pętli), bilans BESS, hosting w czasie | binding serii → atrybuty per krok; SOC jako stan między krokami | `run_annual_oltc_profile` (1 skalar), fantom `run_time_series_power_flow` (usunięty CV-3.2) |
...
**Aktualizacja 2026-09-16 (`../plan/SYNTEZA_DOMKNIECIA_PRODUKTU_2026-09.md` §2, wiążąca):** `stability_rms` NIE jest rozszerzany ani nie zastępuje niczego — audyt wątku badawczego zmierzył brak sprzężenia z siecią (U ≡ 1,0; `enm_ref`/`integrator`/`target_ref` nieczytane; 0 konsumentów), a kontrakt zakodował rozprzężenie. Rdzeń dynamiki = **nowy pakiet `network_model/solvers/dynamika/`** obok rdzeni FROZEN (DT-9): inicjalizacja z rozpływu assemblera → punkt pracy → stany regulatorów → bramka ‖f(x₀,y₀)‖; DAE półjawne indeksu 1 na Ybus; zdarzenia z dokładnym czasem i re-inicjalizacją (stany różniczkowe trzymane, algebra od nowa); trapez niejawny; ograniczniki z parametrami z katalogu (bez domyślek); tożsamość scenariusza; drabina walidacji C×W z wyrocznią ANDES w osobnym środowisku. Wynik = osobny kontrakt `ResultSetDynamicV1` w tym samym rejestrze biegów (A-13). `stability_rms` i `frt_hvrt` usuwane po W6-5 za zgodą B-01 (OD-20); do tego czasu ich wyniki mają stopień dowodowy `UNVALIDATED_MODEL`/`NOT_SIMULATED` i nie są raportowalne (A-2/A-6). Kolejność W6-0…W6-8: synteza §2.11.
