...
| 6 | Power Quality & Dynamics | flicker (cytowany IEC/TR 61000-3-7), VUF z rozpływu niesymetrycznego (W5-D), rdzeń RMS/DAE `solvers/dynamika/` (W6-1…W6-3, W6-A, W6-F R10), ekran Jakość | stabilność dynamiczna = fasada progowa; `ResultSetDynamicV1` bez konsumenta UI; brak ROCOF, zdarzeń warunkowych, zamknięcia NOP, odcinka beznapięciowego; brak raportu EN 50160; **harmoniczne: nie „pod akademickimi", lecz fizycznie błędne (korekta 2026-09-23, §3.6 #4) — REWRITE**; supraharmoniczne: brak | DO-PRZEPROJEKTOWANIA | W2, W6 (Program A/B: `PROGRAM_AB_DYNAMIKA_I_JAKOSC_ENERGII_2026-09.md`) |
...
| 4 | Harmoniczne / THD / TDD / rezonans | **DO-PRZEPROJEKTOWANIA** (korekta 2026-09-23 z audytu `../evidence/OPUS_AUDYT_HARMONICZNE_SUPRAHARMONICZNE_2026-09-23.md` §0.1: wcześniejsza klasyfikacja CZĘŚCIOWE była zawyżona — macierz Y w omach bez przekładni daje U_5 po stronie nN ×1406 za duże (THD_U 463,9 %), R transformatora bez U² (×225), szyna odniesienia = pierwsza na liście (wynik zależy od kolejności danych), moc zwarciowa sieci nieczytana, źródła sumowane z fazą 0°, grupa połączeń ignorowana, `voltage_unbalance=0.0` i „zgodny" bez obliczenia; jedyny test referencyjny sprawdza `THD ≥ 0`; kondensatory ENM nie wchodzą do mostu V12.6; 0 rekordów katalogu przekształtników z widmem. Następca: pakiet `solvers/harmoniczne/` wg `PROGRAM_AB_DYNAMIKA_I_JAKOSC_ENERGII_2026-09.md` AB-2H; zdolność wycofana z powierzchni w AB-1d_min) | `v126_academic.py:240-520` (18 rzędów, limity EN 50160 / IEEE 519); **B-02 (2026-09-10):** karta „Jakość energii i harmoniczne" w katalogu „Analizy specjalistyczne" (grupa „Jakość energii i stabilność przekształtników", podstawa PN-EN 50160 / IEEE 519 cytowana), gotowość nazywa osobno brak karty przekształtnika (bez remedium formularzem) i brak widma (klucz `harmonic_spectra`) | brak interharmonicznych; karta pod „Analizy specjalistyczne", nie pod ekranem „Jakość"; odwzorowanie kondensatorów/filtrów w skanie niezweryfikowane | wysoka/niska |
...
| OD-20 | **B-01 pakiet W6 (synteza 2026-09-16 §4.2):** (a) ewaluator T14/T15/T16/T17/T18 w `solvers/ncrfg_ptpiree/engine.py` przepięty na konsumpcję kanonicznego wyniku czasowego `ResultSetDynamicV1` (koniec tautologii `margin = x − x ≡ 0`, `engine.py:646-655`); (b) kasacja `network_model/solvers/stability_rms/**` i `network_model/solvers/frt_hvrt/**` po W6-5 (zastąpione pakietem `network_model/solvers/dynamika/`); (c) opcjonalnie `capability_id` w `TEST_CATALOG` (mapowanie żyje w rejestrze `solver_input/provenance.py`, więc (c) jest wygodą). Do czasu zgody: bezpiecznik A-6 (wynik T14/T15 = `NOT_SIMULATED` → `not_reportable`, certyfikat nie powstaje) | edycja `network_model/solvers/**` |
| OD-21 | **Dokumenty źródłowe wymagań WOS 2025** (WOS = **wymogi ogólnego stosowania**, art. 7 rozporządzenia 2016/631 — korekta 2026-09-23 po audycie `../evidence/OPUS_AUDYT_WARSTWY_REGULACYJNEJ_2026-09-23.md` §1.3 p. 4: wcześniejsze rozwinięcie „właściwy operator systemu" kolidowało z rejestrem PTPiREE `wos_version` i z solverem; właściwego operatora systemu pisze się w pełni): programy ramowe testów PTPiREE do Procedury testowania wer. 3.0 (obowiązuje od 2026-01-01), IRiESD 2025 pięciu OSD z datami obowiązywania, warunki przyłączenia dla profili — właściciel dostarcza albo potwierdza pobranie ze stron publicznych OSD; do czasu: profile `catalog/profiles/nc_rfg/*.yaml` z adnotacją „wersja niezweryfikowana ze źródłem" (A-15 wersjonowanie) | dane spoza repo; treści normatywnych nie fabrykujemy |
