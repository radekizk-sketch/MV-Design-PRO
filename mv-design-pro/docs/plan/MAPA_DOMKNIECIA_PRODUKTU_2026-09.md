...
| 8 | Pomiary CT/VT | ISTNIEJE-ZWERYFIKOWANE | `enm/models.py:617-696,1092-1130`; `domain/dobor_przekladnika.py` (907; 42 testy); **W3-B (2026-09-09):** `Measurement.obwod_wtorny: ObwodWtorny | None` (długość/przekrój przewodu, obciążenia aparatów, moc styków — kształt 1:1 z `api/equipment_checks.py`) i `vt_uzwojenie` (VT-only, walidowane w `_validate_ctvt_variant_matches_measurement_type`) — addytywne, bez fizycznych wartości domyślnych; operacja `set_measurement_secondary_circuit` zarejestrowana w `CANONICAL_OPERATIONS` i `V2_CANONICAL_OPS`; pin parytetu literałów FE↔BE 115 → 116 | — | wysoka |
...
| 12 | Punkt neutralny i uziemienie | ISTNIEJE-ZWERYFIKOWANE (po W5-A 2026-09-16: JEDEN typ `GroundingConfig` na DWÓCH nośnikach) | `Transformer.hv_neutral/lv_neutral` (→ Y0 `enm/zero_sequence_transformer.py`) + NOWE `Source.neutral_grounding` (kreator źródła wyprowadza r0/x0 z `Z_0 = Z_T0 + 3·Z_N`, `network_model/pochodne/skladowe_zerowe.py`, ślad `zero_sequence_provenance`); SKASOWANE z migracją przy wczytaniu (`enm/uziemienie.py`): `Bus.grounding`, `substation.meta["grounding"\|"zero_sequence"]`, `domain/grounding.py`; jeden słownik `TYPY_PUNKTU_NEUTRALNEGO` (`BayEarthFaultPath.neutral_grounding_mode`, `MvNeutralGroundingItem.grounding_type`, FE `types/uziemienie.ts` pin do OpenAPI); walidator E-W5-01/E-W5-03; `v126_contracts` czyta Source/Transformer; testy `tests/enm/test_uziemienie_migracja.py`, `test_validator_w5_uziemienie.py`, `test_domain_ops_w5_uziemienie.py`, `test_hash_w5_uziemienie.py`, `test_store_migracja_uziemienia.py` | pozostałość ŚWIADOMA: `mv_neutral_grounding_ref` (magazyn audytu 2, tor równoległy — poza W5-A), `v126_academic.py` fallbacki (FROZEN, OD-24); `BayEarthFaultPath.earth_electrode_resistance_ohm` nadal 0 pisarzy (R_u stacji → W5-E) | wysoka |
...
| 10 | Tryby DER Q_CONST/COSPHI/COSPHI_P/Q(U) + LFSM | ISTNIEJE-ZWERYFIKOWANE | `power_flow_inverter.py:49-54,76`; assembler 632-648; 45 testów | brak śladu trybu/limitu Q w wyniku FROZEN (domena 5 #1). **W3-H (2026-09-10):** potwierdzone biegiem na wszystkich trzech metodach (NR/GS/FD) i przez tor kanoniczny `execute_run` — OD-15(g) | wysoka |
...
| 17 | Dobór uziemienia punktu neutralnego (Ic, Petersen, NER) | ISTNIEJE (bez wyroczni niezależnej) | `network_model/solvers/v126_academic.py:1299-1420` (21 testów); ekran „Analizy specjalistyczne" — **B-02 (2026-09-10):** karta z pytaniem, zakresem, wielkościami (I_C, C_0, L/X_L, I_res, R_N, E) i podstawą (IEC 62271-203 / IEC 60364), gotowość z modelu (b0 linii i kabli, napięcie sieci, schemat uziemienia PROPONOWANY z uziemienia szyny/transformatora, pola rezystora wymagane tylko dla schematu rezystorowego) | dwie niepowiązane liczby: Z0 z rdzenia FROZEN vs Ic z modułu akademickiego | średnia |
...
| 2 | Metodyka nastaw nadprądowych | SKONSOLIDOWANE (W3-C1 + W3-C2; było ZDUPLIKOWANE ×3) | `application/protection_settings/{engine.py,batch_run.py}` (Hoppel/IRiESD) — JEDYNA metodyka: `oblicz_nastawy()` współdzielona przez pakiet ZIP (`GET /analysis-runs/{id}/pakiet-dowodowy-nastaw`) i nowe trasy JSON `GET /analysis-runs/{id}/nastawy[/dopasowanie]`, ekran `ui2/wyniki/koordynacja/SekcjaNastaw.tsx` na kanonie (kotwica → odcinek/szyna → jawne parametry c_min/Δt/k_b/k_bth → tabela wyników → ZIP → dobór aparatu z nowym polem katalogu `t_51_s_min/max`); **W3-C1 (2026-09-09):** V12K-189 (`overcurrent/**` 9 plików, `api/protection_overcurrent_settings.py`, ekosystem koperty biegu `run_registry.py`/`run_envelope.py`/4× `envelope_adapter.py`) SKASOWANY — 20 plików, 0 konsumentów produkcyjnych (grep), predykat sparowany `dostepnosc_pakietu_nastaw`/`zbuduj_wejscie_nastaw` na jednym źródle c_max; **W3-C2 (2026-09-09):** FIX-12D (`line_overcurrent_setting/**` 1880 linii + 841 linii testów) SKASOWANY, silnik Hoppla rozszerzony ADDYTYWNIE o `generacja_lokalna` (próg jawny, brak = NIEDOSTĘPNY, bez wartości domyślnej) i `okno_nastaw` (test tożsamości PRZED/PO 7/7), wzorzec RP-LINE-I2-THERMAL-SPZ liczony tą samą metodyką (fixture A/B/C przepisane, nowy D „generacja lokalna"; 8 różnic FIX-12D→Hoppel z liczbami PRZED/PO; `_secondary_a` skasowane — Hoppel bez przekładni CT) | bramki wskrzeszenia `check_w3c1_overcurrent_resurrection` / `check_w3c2_line_overcurrent_setting_resurrection`; poza wzorcem (nazwane): I> opóźnione i ogólne `thermal.is_adequate` (Hoppel liczy, wzorzec „I>>" nie prezentuje); złote hashe `nastawy/*` parytetu przeliczone (hash całego `ProtectionSettingsInput` zmienia się z definicji przy polu addytywnym; pole `parametry` identyczne PRZED/PO) | wysoka |
| 3 | Kryterium ALF / nasycenie CT | SKONSOLIDOWANE (W3-B; było ZDUPLIKOWANE ×2) | `equipment_checks/ct_burden_saturation.py::check_ct_burden_saturation` (jądro FROZEN, ALF_eff = ALF·(Sn+Sw)/(S2obl+Sw)) = jedyny werdykt nasycenia; **W3-B (2026-09-09):** `domain/dobor_przekladnika.py` kryterium `ct.alf` deleguje w 100 % do jądra (addytywne `Kryterium.kody_gotowosci`/`slad` WHITE BOX), dawny warunek konieczny zostaje jako `ct.alf_katalogowy` z werdyktem wyłącznie `informacja`/`brak_danych`; obwód wtórny z modelu (`Measurement.obwod_wtorny`, `vt_uzwojenie`; `add_ct`/`add_vt` + operacja `set_measurement_secondary_circuit` — jedyna droga edycji na kolekcji `measurements`); parytet end-to-end z `POST /api/solver/ct-burden-check` na realnym katalogu; testy iloczynu cech {obwód zapisany/brak} × {Rct} × {ALF z pola/klasy/brak} × {Ik″} z realnymi kodami gotowości jądra | pole wytwórcy (`api/generators.py:1056,1074`) nadal `obciazenie_obwodu_va=None` z NAZWANYM powodem: CT/VT pola wiązane przez `set_der_catalog_bindings`, bez elementu `Measurement` — częściowa fizyka byłaby gorsza niż uczciwy brak | wysoka |
...
| 5 | Dobór CT/VT | CZĘŚCIOWE | jak domena 2 #3; **W3-B (2026-09-09):** kreator stacji (`ui2/kreatory/stacja`) przyjmuje obwód wtórny CT/VT, `ui2/kryteria/SekcjaBilansuCtVt.tsx` i `DoborPrzekladnikowSekcja.tsx` pokazują kody gotowości jądra jako chipy z akcjami naprawczymi i rozwijalny ślad WHITE BOX; e2e `kreator-stacji-max.spec.ts` 2/2 | tylko pole wytwórcy — bez elementu modelu CT/VT dla S2obl (patrz #3) | wysoka |
| 6 | 50/51 | ISTNIEJE-ZWERYFIKOWANE (fizyka skonsolidowana: IDMT W3-A, nastawy W3-C1/W3-C2) | model + `czas_wylaczenia_*` + koordynacja + `ui2/wyniki/koordynacja` (od W3-C1 z sekcją nastaw na kanonie Hoppla) | — | wysoka |
...
| # | Zdolność | Klasyfikacja | Dowód | Brakuje / uwaga | Pewność |
|---|---|---|---|---|---|
| 1 | PV w ENM + rozpływ | CZĘŚCIOWE | `enm/models.py:493-538`, `power_flow_inverter.py:49-90`; kreator `ui2/kreatory/zrodlo-oze/` | `PowerFlowResult` nie niesie, który tryb zadziałał ani czy limit Q osiągnięto. **W3-H (2026-09-10, zamknięte wariantem B):** potwierdzone biegiem realnym — nawet ślad solvera Newtona (`InverterShaping`/`_inverter_sources_trace`, wyłącznie `trace_level="full"`) niesie tryb/Q/U bez limitu i flagi ograniczenia, a tor kanoniczny (`_build_power_flow_trace_steps`) odrzuca cały rekord przed zapisaniem przebiegu (GS/FD nie budują go wcale) → wykres „Q z biegu vs prawo" niemożliwy bez B-01; wpis OD-15(g). Ekran `ui2/wyniki/rozplyw` ma uczciwy stan zerowy (podzakładka „Regulacja Q OZE") | wysoka |
...
| 4 | Harmoniczne / THD / TDD / rezonans | CZĘŚCIOWE | `v126_academic.py:240-520` (18 rzędów, limity EN 50160 / IEEE 519); **B-02 (2026-09-10):** karta „Jakość energii i harmoniczne" w katalogu „Analizy specjalistyczne" (grupa „Jakość energii i stabilność przekształtników", podstawa PN-EN 50160 / IEEE 519 cytowana), gotowość nazywa osobno brak karty przekształtnika (bez remedium formularzem) i brak widma (klucz `harmonic_spectra`) | brak interharmonicznych; karta pod „Analizy specjalistyczne", nie pod ekranem „Jakość"; odwzorowanie kondensatorów/filtrów w skanie niezweryfikowane | wysoka/niska |
...
| 6 | Ekran „Jakość" (sanity, energia, profil, arc flash, estymacja) | ISTNIEJE-ZWERYFIKOWANE | `api/quality_analysis_runs.py:43-56`; `ui2/wyniki/jakosc/EkranJakosci.tsx` (107 testów). **W3-J (2026-09-16):** widok profilu napięcia (`application/analyses/voltage_profile_view.py`) i widok wyniku rozpływu (`api/canonical_run_views.py`) niosą addytywnie `kryteria_napiecia` z jedynego źródła `analysis/normative/kryteria_napiecia.py` — front bez własnych progów, starsze zapisane biegi bez tego pola pokazują uczciwy stan „kryterium niedostępne", nie liczbę domyślną | nazwa „jakość" = kontrola wiarygodności wyniku, nie PQ | wysoka |
...
| 11 | Wybór układu sieci nN | ISTNIEJE-ZWERYFIKOWANE (po W5-A 2026-09-16) | `Transformer.lv_earthing_system: UkladSieciNn \| None` (`enm/models.py`, jedna definicja literału; enum solvera FROZEN przez `solver_input/uklad_sieci_nn.py::typ_sieci_solvera` + test równości zbiorów); E063 na transformatorze (`enm/uklad_sieci_nn.py::transformatory_bez_ukladu_nn` — JEDEN predykat dla walidatora, `eligibility_service` i `readiness_bridge`); żywy `ui2/kreatory/stacja` pisze pole typowane (`uklad_sieci_nn`); `meta["nn_earthing_system"]` SKASOWANE z migracją przy wczytaniu; `_DEFAULT_SYSTEM="TN-C-S"` SKASOWANE (brak = odmowa `BRAK_UKLADU_NN` w pętli zwarcia); `ui/topology/EarthingSystemSelector.tsx` SKASOWANY (+ `EarthingInspector.tsx`, `earthingTypes.ts`, `EarthingBadge.tsx`); zapadka `scripts/grep_zero_guard.py` grupa `kod_` + self-test | — | wysoka |
...
| 4 | `PROTECTION_OVERCURRENT` (`protection_settings`) pack | ISTNIEJE-ZWERYFIKOWANE (rozszerzone W3-C1, 2026-09-09) | `pakiet_nastaw.py` — `oblicz_nastawy()` (`batch_run.py`, jedna funkcja) współdzielona przez ZIP (`GET /analysis-runs/{id}/pakiet-dowodowy-nastaw`) ORAZ nowe trasy JSON `GET /analysis-runs/{id}/nastawy[/dopasowanie]` (422 na `PakietNastawError`, `canonicalize_json`); konsument `ui2/wyniki/koordynacja/SekcjaNastaw.tsx`; e2e realny backend `nastawy-koordynacji-hoppel.spec.ts` | KOREKTA inwentarza 2026-08-08 (§6); nazwa `PROTECTION_OVERCURRENT` to koincydencja nazewnicza — pack dotyczy metodyki `protection_settings` (Hoppel), NIE skasowanego pakietu `overcurrent/` | wysoka |
...
| A5 | Stała strona oceny właściciela (zrzuty żywej aplikacji, oba motywy) i pętla ux-feedback | CZĘŚCIOWE | `frontend/e2e/ux-feedback-loop.spec.ts` (mock backend), `creator-screenshot.spec.ts`, `wszystkie-sceny-screenshot.spec.ts`; `docs/audit/visual/ocena-2026-07/` 7 PNG, ostatni commit 2026-07-24; **B-02 (2026-09-10):** sceny `akademickie`/`ocena`/`ocena-przekroczenia` w bramce `wszystkie-sceny-screenshot.spec.ts` (oba motywy, `docs/audit/visual/sceny/`) + zrzuty z ŻYWEJ aplikacji na realnym backendzie `v126-okna-zrzuty.spec.ts` (`docs/sld/audyt-2026-08/v126-*.png`) | brak „stałej strony oceny" z reguły 8 dyrektyw właściciela; zrzuty statyczne od lipca; W1 DoD #11 i B-02 (zrzuty do werdyktu) dostarczane kanałem sesji, nie repo (PNG niecommitowane) | wysoka |
...
| D4 | Granice zdrowego rozsądku wyników | ISTNIEJE-ZWERYFIKOWANE | `analysis/sanity_bounds/short_circuit_bounds.py` (133) + `analysis/sanity_bounds/power_flow_bounds.py` (pasma napięcia/obciążenia/strat rozpływu — W3-G2), `GET /api/quality/sanity-bounds`, `ui2/wyniki/jakosc/EkranJakosci.tsx`; 96 testów | zwarcia I rozpływ pokryte (korekta wobec poprzedniej treści wiersza — „tylko zwarcia" było nieaktualne od W3-G2). **W3-J (2026-09-16):** pasmo napięciowe rozpływu (`PASMO_NAPIECIA_PROCENT`) było DRUGIM niezależnym źródłem liczby 10,0 — skasowane, czyta bit w bit `analysis/normative/kryteria_napiecia.py::PASMO_WIARYGODNOSCI_PROCENT` (jedno źródło z resztą klasy: `NormativeConfig`, `energy_validation`, `reactive_adequacy`, `power_flow_interpretation`); parytet PRZED/PO zweryfikowany na sieci złotej (regeneracja fikstury `16_stale_result.json` jedyną zmianą w 18-scenariuszowym zestawie nN) | wysoka |
...
Wycinki: D2/D7/D8 → **W3**; D1 → **W8** (koperta biegu z wyrocznią); D9 → **W11**; D10 → program 10x.
D4 ZAMKNIĘTE w W3 (W3-G2 pasma rozpływu + W3-J jedno źródło pasma napięciowego, 2026-09-16) — zdjęte z listy wycinków.
...
| E5 | Konwersje jednostek w łańcuchu danych | ISTNIEJE-ZWERYFIKOWANE (W3-F) | `network_model/pochodne/jednostki.py` (20 funkcji z sufiksem jednostki, jedno recenzowane miejsce); `backend_no_physics_guard.py` rodzina `J_skalowanie_jednostek` (AST: `BinOp` Mult/Div ze stałą ∈ {1000, 0,001, 10⁶, 10⁻⁶} i operandem o sufiksie jednostki); **W3-F (2026-09-09):** 153 wzorce w 54 plikach przeniesione bit w bit (testy tożsamości), pomiar PRZED 192/59 plików → PO scaleniu fali 2 (2026-09-10) **1 plik / 2 wzorce**; `api/solver_input.py:202,225` → `solver_input/builder.py:85,123,414,458` wątkuje częstotliwość studium (`header.defaults.frequency_hz`) do `susceptancja_katalogowa_us_per_km` (B = 2πfC) — jedyny produkcyjny wołający | zapadka NIE jest pusta i taka zostanie: 1 wpis TRWAŁY `application/result_mapping/short_circuit_to_resultset_v1.py` (2×; plik pod `PROTECTED_FILES` `resultset_v1_schema_guard.py`, B-01 — F.1 omyłkowo go zmigrował, cofnięte bajt w bajt); `Branch.resolve_electrical_params`/`with_resolved_params` (`core/branch.py:501,544`) — 0 wołających produkcyjnych (zaimplementowane, przetestowane, niewpięte — osobna karta); propagacja częstotliwości projekt → ENM: `api/projects.py` nie dotyka `get_enm`/`set_enm` — brak trasy zapisu (osobna karta) | wysoka |
...
| F9 | Uziemienie ekranu, żyła powrotna, napięcia indukowane | CZĘŚCIOWE (po W5-A 2026-09-16: uziemienie ekranu ISTNIEJE; napięcia indukowane BRAK z nazwanym powodem) | żyła powrotna liczona realnie w pętli zwarcia nN (`fault_loop_builder.py:60-117`, `fault_loop_iec60364.py:165-243`); uziemienie ekranu = pole typowane `Cable.screen_bonding ∈ {single_end, both_ends, cross_bonded}` (WSZYSTKIE operacje tworzące kabel SN/nN + podział odcinka + `update_element_parameters`, kreatory ui2 magistrala/odgałęzienie/pierścień/odcinek nN/tor SN DER), `CableType.z0_reference_bonding` w katalogu, ostrzeżenie `W-W5-01` (układ modelowany ≠ referencyjny katalogu), pisarz `earthing_role="cable_screen"` w kreatorze pola SN (`RolaUziemnika`); 0 trafień napięć indukowanych | napięcia indukowane i straty w ekranach (IEC 60287-1-1) wymagają geometrii ułożenia, której katalog nie niesie → W10 razem z doborem przekroju; `LVCableType` bez `z0_reference_bonding` (katalog nN nie deklaruje r0/x0 ekranu) | wysoka |
...
| G3 | Straty transformatora ΔP(β) = P0 + β²·Pk | JEDYNA LICZBA NA POWIERZCHNI (W3-E; kod duplikatu w solverze FROZEN) | kanon `solvers/equipment_checks/transformer_losses.py:53-161` → `POST /api/solver/transformer-losses` → `ui2/kryteria/SekcjaKartyKatalogu.tsx:162-252` (28 testów); **W3-E (2026-09-09):** duplikat `v126_academic.py::_opf_loss_lcc` (β = 0,45 i `oltc_tap_position: 0` zaszyte) zdjęty z powierzchni — `POST …/runs/v126/opf_loss_lcc` → 410 z zamiennikiem, kafel OPF/LCC poza listą wyboru (12 → 10) | kod w solverze FROZEN = OD-15(d) (patrz 9 #7) | wysoka |
...
| G6 | Grupa połączeń i model składowej zerowej | ISTNIEJE-ZWERYFIKOWANE (po W5-A 2026-09-16, F-4) | `enm/zero_sequence_transformer.py` (tablica połączeń wg IEC 60909-0; dostępność punktu neutralnego z LITERY grupy — config na uzwojeniu bez N/n = odmowa nazwana, nie pierwszeństwo configu) → `enm/mapping.py` → `enm/assembler.py`; słownik IEC 60076-1 `enm/grupa_polaczen.py` (46 grup) = jedno źródło dla walidatora (`E-W5-02`, `E-W5-03`), OpenAPI (`GET /api/catalog/grupy-polaczen`) i trzech pól wyboru frontu (kreator transformatora ui2: wartość z katalogu + zmiana z listy → `add_transformer_sn_nn.vector_group`; `StationConfigTransformerCard.tsx` select wpięty do `update_element_parameters` — dotąd phantom; edycja parametrów); odmowa przy zapisie `transformer.invalid_vector_group`; domyślne `"Dyn11"` w `catalog/types.py` i `property-grid/field-definitions.ts` skasowane | pozostałość ŚWIADOMA: `enm/mapping.py` domyślne `"Dyn11"` dla gałęzi bez grupy (zachowanie rdzenia FROZEN `transformer_phase_shift_rad`, SM-2/V12K-180) | wysoka |
...
| J4 | Testy zgodności NC RfG / PTPiREE (macierz per DER, zgodność przekrojowa per przypadek) | ISTNIEJE-ZWERYFIKOWANE (W3-D) | `solvers/ncrfg_ptpiree/engine.py` (972, FROZEN), 5 profili OSD; `POST /api/ncrfg-tests/run`, `GET /catalog`, `GET /cases/{id}/compliance`; `ui2/oze/macierz/MacierzNcRfg.tsx` → `/catalog` + `/run`; **W3-D (2026-09-09):** `SekcjaZgodnosciPrzekrojowej.tsx` czyta `GET /cases/{case_id}/compliance` na żywo (stany brak_przypadku/ładowanie/brak_der/gotowe/błąd), jeden stan operatora z macierzą per DER; 12 + 51 testów + testy sekcji; trzecia ścieżka oceny (`source_compliance`) skasowana | — (= 5 #12) | wysoka |
| J5 | Krzywe regulacji Q(U)/cosφ(P) i trajektoria FRT | CZĘŚCIOWE | FRT: `ui2/oze/frt/WykresTrajektoriiChart.tsx:157` z realnego biegu (`frt_trajektorie.py`); Q(U)/cosφ(P): `ui2/kreatory/zrodlo-oze/WykresyNcRfg.tsx` (246; SVG kreślony z nastaw użytkownika, jawnie „ZERO fizyki sieci") = podgląd prawa sterowania, nie wynik; `ui2/oze/krzywe/` = obwiednia P–Q (`pq_area.py`), nie krzywe regulacji; **W3-D (2026-09-09):** trzecia, uboższa ścieżka oceny FRT/Q(U)/cosφ(P) (`application/compliance/source_compliance.py`, porównanie punktów bez modelu dynamicznego, niespójne kryterium) SKASOWANA — ocena wyłącznie kanonem NC RfG (J4) | brak wykresu „Q wstrzyknięte w biegu vs prawo Q(U)" (= 5 #1: `PowerFlowResult` nie niesie trybu ani limitu Q) → W3-H. **W3-H zamknięte 2026-09-10 (wariant B, zero fabrykacji):** badanie z dowodem potwierdza brak (OD-15(g)); ekran `ui2/wyniki/rozplyw` ma uczciwy stan zerowy zamiast wykresu | wysoka |
...
| J8 | Proweniencja parametrów falownika w biegu | ISTNIEJE-ZWERYFIKOWANE | `Generator.source_mode` (KATALOG/MIGRACJA/EKSPERCKI_RECZNY); `InverterSource.k_sc_zrodlo` (KATALOG/ZALOZENIE) do śladu WHITE BOX (`mapping.py:807-823`); readiness `inverter.k_sc_assumed` (WARNING) / `inverter.k_sc_missing` (BLOCKER) | **W3-I (2026-09-09):** predykat „element wymaga katalogu" = jedna tabela `wymagalnosc_katalogu` (patrz K8); generator przekształtnikowy bez katalogu dostaje kod walidatora `W010` (WARNING) odwzorowany na `inverter.k_sc_missing` — brak zgadywania k_sc przy CAŁKOWITYM braku katalogu (pierwsza wersja komunikatu obiecywała 1,1 — poprawione) | wysoka |
...
| K8 | Bramka katalogowa wszystkich dróg wejścia | ISTNIEJE-ZWERYFIKOWANE (jeden predykat, W3-I) | **W3-I (2026-09-09):** jedna tabela `wymagalnosc_katalogu(rodzaj, tworzenie/walidacja/import)` (`catalog/governance.py:202-344`, poziomy BLOCKER/WARNING/NIE per rodzaj, z wyjątkiem `MANUAL_EQUIVALENT` dla źródła) czytana przez 8 miejsc: walidator E009/I002 + NOWY kod `W010` (generator przekształtnikowy bez `catalog_ref`, WARNING, odwzorowanie `readiness_bridge.py` → `inverter.k_sc_missing`), brama ZIP (`project_archive/service.py`, +źródła), CGMES side-car (`cgmes_importer.py`, wyjątek MANUAL_EQUIVALENT — wcześniej brak), XLSX (`xlsx_import/importer.py`, asercje zależności od tabeli), projekcja v2 (`enm/v2_projection.py`: lokalna kopia myliła klasyfikację zwarciową 4 `gen_type` z klasyfikacją DER 6 — skasowana), gotowość obliczeniowa (`calculation_readiness/service.py`), `eligibility_service.py::_check_catalog_refs` (7. konsument, nienazwany w karcie), `domain/readiness_bridge.py` (8.); testy iloczynu cech + `tests/api/test_katalog_predykat_operacje_parytet.py`; fixtury projekcji nN 16/17 zregenerowane (W010 zgłasza `QF-03_zrodlo`) | `add_generator_sn`: domena wymusza katalog, ale `api/domain_ops_policy.py::CATALOG_REQUIRED_OPERATIONS`, `extract_catalog_binding()` i FE `catalogFirstRules.ts` nie mają tej operacji — rozjazd nazwany w `_ROZJAZD_TWORZENIE_ZNANY` (osobna karta); `add_load_sn` bez katalogu = decyzja architekta, nie dług; grupy „klasa CIM" (`cgmes_exporter.py:441` vs `cgmes/service.py:131`) i „wariant przyłączenia" (`generator_validation.py:30` vs `field_device.py:240`) = inne mechanizmy, nazwane | wysoka |
...
5. **Predykaty parami:** „element wymaga katalogu" w dwóch miejscach (`wymaga_referencji_katalogowej` vs E009,
   J8/K8 — **domknięte W3-I 2026-09-09**: jedna tabela `wymagalnosc_katalogu`, 8 konsumentów); ścieżka zasilania
   BFS ×2 (B1); tabela BIL ×2 (H3 — **W3-E**: test parytetu `test_v126_bil_parytet.py`). Instancje K-A → W3/W7/W6.
...
|---|---|---|
| **K-A Druga prawda sieci** | legacy ORM `network_*` 6 tabel + 3 pisarze/czytelnicy; import XLSX tylko do legacy; ZIP podwójny zapis; `catalog_governance` czyta legacy; `NetworkWizardService`; SLD ORM + `api/sld.py`; stan łączeniowy ×2; uziemienie ×6; tabele katalogu w bazie pisane przez governance, nieczytane przez operacje domenowe (1 #24); N-1 na innej fikturze niż G03; semantyka SLD w kliencie bez ścieżki backendowej; + aneks 3a: predykat „element wymaga katalogu" ×2 (J8/K8 — domknięte W3-I), BFS ścieżki zasilania ×2 (B1), tabela BIL ×2 (H3 — parytet testem W3-E) | W1 (ORM, XLSX, ZIP, governance, wizard, SLD ORM), W5 (uziemienie, stan łączeniowy), W7 (SLD), W8 (G03) |
...
| OD-19 | **Defekt solvera rozpływu Newtona-Raphsona (B-01, zmierzony przez W3-G2, 2026-09-10):** dla sieci ze źródłem SN z jawnym równoważnikiem górnego napięcia (`hv_voltage_kv=110`, `transformer_sn_mva=25`) rozpływ NR zbiega (`converged=true`) do fizycznie zdegenerowanego rozwiązania na szynie „Szyna 110 kV TR1" (U ≈ 1e-17 kV zamiast 110 kV, `slack_p_mw` ≈ 5713 MW) — dowód: spec `frontend/e2e/pasma-rozplywu-jakosc.spec.ts` (sekcja pasm POPRAWNIE melduje „poza zakresem wiarygodności" zamiast fabrykować „zweryfikowany"). Propozycja architekta: diagnoza w rdzeniu `network_model/solvers/power_flow_newton.py` (punkt startowy płaski vs zbieżność do niskonapięciowej gałęzi rozwiązania; kryterium zbieżności bez kontroli fizycznej sensowności U wobec Un), wyrocznia pandapower dla tej topologii, korekta z parytetem golden bit w bit dla wszystkich sieci rejestru | edycja `network_model/solvers/**` |
| OD-20 | **B-01 pakiet W6 (synteza 2026-09-16 §4.2):** (a) ewaluator T14/T15/T16/T17/T18 w `solvers/ncrfg_ptpiree/engine.py` przepięty na konsumpcję kanonicznego wyniku czasowego `ResultSetDynamicV1` (koniec tautologii `margin = x − x ≡ 0`, `engine.py:646-655`); (b) kasacja `network_model/solvers/stability_rms/**` i `network_model/solvers/frt_hvrt/**` po W6-5 (zastąpione pakietem `network_model/solvers/dynamika/`); (c) opcjonalnie `capability_id` w `TEST_CATALOG` (mapowanie żyje w rejestrze `solver_input/provenance.py`, więc (c) jest wygodą). Do czasu zgody: bezpiecznik A-6 (wynik T14/T15 = `NOT_SIMULATED` → `not_reportable`, certyfikat nie powstaje) **Decyzja i stan po AB-1a (2026-09-23):** zgoda z delegacji właściciela 2026-09-22 — plan A/B decyzja O-5 (edycje `solvers/ncrfg_ptpiree/**` z parytetem w testach klasy; pozycja (c) bramki B-01 w planie §12.2). (c) **wykonane w AB-1a Pakiet C:** `TEST_CATALOG` niesie `zdolnosc_id` i `rodzaj_twierdzenia`, mapa `solver_input/dowod_ncrfg.py` skasowana; (a) **tautologia usunięta, konsumpcja wyniku czasowego w AB-1c:** T14–T18 bez biegu dynamiki = rekord `OcenaKryterium` o statusie `NIE_OCENIONO` z nazwanym brakiem (kontrakt V2 solvera, O-13/O-34) — bezpiecznik A-6 zastąpiony kompletnością dowodu rekordów, certyfikat czyta wyłącznie rekordy `WynikWymagania`; przepięcie na kanoniczny wynik dynamiki (cztery wymagania FRT: art. 14 ust. 3, art. 20 ust. 2 lit. b i c, art. 20 ust. 3 lit. a, art. 17 ust. 3) — przyrost AB-1c po AB-1b.1/1b.3/1b.2/1d_min (sekwencja §5 planu); (b) w AB-1a werdykty konsumentów `frt_hvrt`/`stability_rms` → `NIE_OCENIONO` (Pakiet 0, O-25), kasacja pakietów po migracji ścieżek użytkownika w AB-1c (O-5 (b)). Commity: plan `dc9c55f3`/`fe5b0fef` (O-5, O-13, O-25), kontrakt werdyktu `600d9a8f`/`e2d56712`; kod solvera — commit integracyjny AB-1a (w toku). | edycja `network_model/solvers/**` |
| OD-21 | **Dokumenty źródłowe wymagań WOS 2025** (WOS = właściwy operator systemu, art. 41 NC RfG): programy ramowe testów PTPiREE do Procedury testowania wer. 3.0 (obowiązuje od 2026-01-01), IRiESD 2025 pięciu OSD z datami obowiązywania, warunki przyłączenia dla profili — właściciel dostarcza albo potwierdza pobranie ze stron publicznych OSD; do czasu: profile `catalog/profiles/nc_rfg/*.yaml` z adnotacją „wersja niezweryfikowana ze źródłem" (A-15 wersjonowanie) **Decyzja i stan po AB-1a (2026-09-23):** plan A/B decyzje O-2′ i O-43 — adnotację na poziomie pliku zastępuje **stan źródła KAŻDEJ wartości** (`PodstawaWymagania`: rodzaj, dokument, wydanie, jednostka redakcyjna, `ZWERYFIKOWANE`/`WSKAZANE`/`NIEUSTALONE`) w profilu warstwowym NC RfG → WOS → procedura PTPiREE → WiPWC → OSD → magazyny → wartości zastane (Pakiet B AB-1a; pięć identycznych kopii YAML skasowanych). Liczenie nie jest blokowane; werdykt wobec limitu `NIEUSTALONE` = `BRAK_PODSTAWY`, reguła pokrycia certyfikatem `NIEUSTALONE` = `BRAK_DOWODU`. **Lista danych, których brakuje, jest GENEROWANA z profilu** (`scripts/bramka_danych_profilu.py`, strażnik w `python-tests.yml`): plan A/B §12.1, 58 wierszy na 2026-09-23 — dokumenty warstw OSD pięciu operatorów, magazynów i wartości zastanych, parametry zastane (statyzm, strefa martwa, rampa, zakres Q, obwiednie LVRT/HVRT, odbudowa P), kryteria akceptacji i program badań procedury PTPiREE, reguła pokrycia WiPWC per wymaganie (16), wykonanie prawa operatora (3 wymagania × 5 operatorów), Bank Nastaw, jednostka redakcyjna progów klas WOS. Dołączony dokument podnosi stan bez zmiany kodu. Pozycja pozostaje OTWARTA po stronie właściciela (dokumenty). Commity: plan `dc9c55f3` (O-2′), `fe5b0fef` (O-43); profil warstwowy i bramka danych — commit integracyjny AB-1a (w toku). | dane spoza repo; treści normatywnych nie fabrykujemy |
| OD-22 | **Skutek A-3 (K_sc DEFAULT_FORBIDDEN) dla istniejących projektów z OZE bez kart producentów:** dobór aparatury, nastawy i pakiety dowodowe BLOKOWANE do uzupełnienia deklaracji `k_sc` (uczciwe; wynik zwarciowy nadal liczony i widoczny jako roboczy). Właściciel potwierdza świadomie albo wskazuje okres przejściowy z widocznym znacznikiem „domyślka systemowa" w dokumencie. Domyślnie wdrażana BLOKADA (dyrektywa 2026-09-16) **Pomiar S-2 (2026-09-16, odbiór, drzewo scalone):** BLOKADA wdrożona (kod gotowości `inverter.k_sc_default_forbidden`: `partial` dla biegu zwarciowego, `blocked` dla doboru/nastaw/pakietów); na fiksturach repo 4 z 11 sieci OZE tracą miarodajność dla doboru (rejestr `tests/reference_networks/builders.py`: GN04, GN05, GN06a, GN06b — rekordy katalogowe `conv-pv-nn-0p5mw-0p4kv`/`conv-bess-nn-0p5mw-0p4kv` bez deklaracji `k_sc`), substrat archetypów stacji 0/8 (jawne `k_sc`); bazy projektów użytkowników w tym środowisku nie ma, więc liczba dotyczy fixtur, nie danych produkcyjnych. | wpływ na istniejące projekty użytkowników |
| OD-23 | **PR #471** (Codex, 2026-08-10, `mergeable_state: dirty`): treść (guardy `route_prefix`/`v126_contract_text`/`guardy_z_ci`/`verification_entrypoints`, kasacje `protection-comparison`/`variantStore`) zrealizowana inaczej na gałęzi — PR zastąpiony, do zamknięcia przez właściciela | cudzy PR |
| OD-24 | **Domyślne wartości fizyki w rdzeniu FROZEN `solvers/v126_academic.py`** (karta W5 §1 p. 12; inwentarz `../audit/INWENTARZ_W5_2026-09-16.md` (a)/(c)): fallbacki `neutral_earthing_type` → `"petersen_coil"` (:1306), `"petersen_tuned"` (:1732), `"isolated"` (:1820) oraz stała `"voltage_unbalance_u2_u1": 0.0` (:412). Pytanie: zgoda na edycję rdzenia FROZEN — kasacja fallbacków i stałej (odmowa nazwana zamiast wartości)? W5 pinuje testem, że wejście z modelu (`solver_input/v126_contracts._resolve_network_neutral`) zawsze niesie wartość, niezależnie od decyzji. | B-01: edycja `network_model/solvers/**` wymaga zgody właściciela |
| OD-25 | **Pętla zwarcia TT/IT z RCD — nowy moduł solvera** (karta W5 §1 p. 8): `solvers/fault_loop_iec60364.py` (FROZEN) podnosi `NotImplementedError` dla TT/IT. W5-E buduje model (`rcd`, `earth_electrode_r_ohm`, R_B z `lv_neutral`), katalog RCD i gotowość nazwaną (`fault_loop.tt_it_not_supported`, koniec domyślnego TN-C-S) bez solvera. Pytanie: zgoda na nowy moduł `solvers/petla_zwarcia_tt_it.py` (TT: `R_A·I_Δn ≤ 50 V`, czasy wg IEC 60364-4-41 411.5.3; IT: 411.6.2/411.6.4) w pakiecie FROZEN — jak OD-20 dla `dynamika/`? | B-01: nowy moduł w pakiecie FROZEN wymaga zgody właściciela |
| OD-26 | **Pole `compliance_tests` (identyfikatory `T1`…`T18` dawnego silnika) w zamrożonych profilach operatorów `catalog/profiles/nc_rfg/*.yaml` (18 wpisów) straciło jedynego konsumenta po kasacji `application/ncrfg_compliance/checker.py` (karta S-3, 2026-09-16); zostaje wyłącznie loader i test `tests/catalog/test_pr9_nc_rfg_profiles_and_turbines.py:68-69`.** Kasacja pola i testu wymaga edycji profili YAML (B-01). Do decyzji: zgoda na usunięcie martwego pola z profili (bez zmiany wartości fizycznych) albo pozostawienie jako archiwum z adnotacją **Decyzja i stan po AB-1a (2026-09-23):** kasacja z delegacji właściciela 2026-09-22 (plan A/B §2, karta AB-1a §0 pkt 11) — pole `compliance_tests` skasowane z kontraktu katalogu, API i typów frontendu; pięć plików profili zastąpionych profilem warstwowym, który tego pola nie niesie (jedyna przestrzeń numeracji testów: `TEST_CATALOG` T01–T20 — numeracja repozytorium, `docs/analysis/NC_RFG_PTPiREE_TESTY_KANON.md` §3); test `test_pr9_nc_rfg_profiles_and_turbines.py` przepisany z intencją w komentarzu. Pozostałość do usunięcia przy integracji: docstring `backend/src/domain/der_protection_functions.py:26` wymienia `compliance_tests` wśród kluczy profilu (opis nieaktualny). Commity: karta AB-1a `fe5b0fef`/`906e4a2d`; kod — commit integracyjny AB-1a (w toku). | edycja zamrożonych profili NC RfG (B-01) |
| OD-27 | **Model tozsamosci uzytkownika (auth/perimeter, program 10x F1)** — przeniesione z rejestru dlugu V12T-015 (2026-09-17). POMIAR (2026-09-16): `backend/src/api/main.py` rejestruje 44 routery, ZERO tras uwierzytelniania (`auth`, `/me`, `current_user`, `login`); powloka skaluje magazyn ustawien prefiksem `userScope` = stala `local`, wiec ustawienia ukladu (szerokosci paneli, tryb zaawansowania, aktywna przestrzen) sa wspolne dla WSZYSTKICH uzytkownikow tej samej przegladarki, a dziennik zmian modelu nie ma czym podpisac autora. Pytanie do wlasciciela: czy produkt ma miec konta i role (kto uruchomil bieg, kto zatwierdzil dokument), czy pozostaje jednostanowiskowy? Odpowiedz wyznacza zakres F1 (sesje, role, audyt autorstwa) i przesadza, czy `userScope` znika, czy staje sie realnym identyfikatorem konta. | decyzja produktowa o zakresie (konta/role/audyt autorstwa), nie do rozstrzygniecia z repo |
...
| OD-5 (stan) | Progi mocy modułów wytwarzania NC RfG (klasyfikacja A/B/C/D): profile niosły maksima NC RfG dla Europy kontynentalnej (A/B 1 000 kW, B/C 50 000 kW) zamiast progów krajowych, a reguła napięciowa typu D działała jako `> 110 kV` (źródło: `docs/evidence/CONVERGENCE_EVIDENCE.md`, wiersz OD-5) | **Rozstrzygnięte — plan A/B decyzja O-1 (2026-09-22, z delegacji właściciela), stan po AB-1a (2026-09-23):** progi krajowe WOS (decyzja Prezesa URE 2018) A 0,8 kW ≤ P < 200 kW, B 200 kW ≤ P < 10 MW, C 10 MW ≤ P < 75 MW, D P ≥ 75 MW albo napięcie przyłączenia ≥ 110 kV (włącznie) w warstwie `warstwy/wos.yaml`; maksima NC RfG zostają w `warstwy/nc_rfg.yaml` jako granice dopuszczalne (loader odrzuca próg krajowy powyżej granicy). JEDNA klasyfikacja w produkcie `klasyfikacja_modulu(p_max_kw, napiecie_kv)` z podstawą progów i powodem (solver, API `/modul` w kW, walidacja generatora, dokument studium); klasyfikacja w interfejsie (`rankingModel.ts`, `studiumModel.ts`) i moduł `compliance/nc_rfg_modul.py` skasowane. Podstawa progów ma stan `NIEUSTALONE` (jednostka redakcyjna dokumentu WOS do wskazania) — klasyfikacja ważna i oznaczona; brak widoczny w bramce danych właściciela (plan A/B §12.1, wiersz `klasyfikacja_zrodlo`). Commity: plan `dc9c55f3` (O-1); kod — commit integracyjny AB-1a (w toku). |
| OD-33 (stan) | Która obwiednia FRT jest normatywna (trzy reprezentacje: rejestr profili operatorskich, stała toru stabilności `application/stability/voltage_trajectory.py`, kopia frontowa `frtEnvelopeValidator.ts`; `FINAL_DYNAMICS_CAPABILITY_FREEZE.md` §7/§7.1, `docs/audit/FRT_PROWENIENCJA_NORMATYWNA.md`) | **Rozstrzygnięte — rozstrzygnięcie właściciela 2026-09-18 („jedno kanoniczne, wersjonowane źródło prawdy") wdrażane decyzją O-4 planu A/B; stan po AB-1a (2026-09-23):** jedna obwiednia na profil, w profilu regulacyjnym, z proweniencją per obwiednia (`NcRfgVoltageLevels`: punkty LVRT/HVRT, zakres typów modułu, `lvrt_zrodlo`/`hvrt_zrodlo`; LVRT z warstwy wartości zastanych, HVRT jako nadpisanie OSD czterech operatorów) — stan źródła `NIEUSTALONE` (proweniencji normatywnej brak w repozytorium), więc kryteria wobec obwiedni dają `BRAK_PODSTAWY`/`NIE_OCENIONO` z zastrzeżeniem, a wiersze `voltage_levels.*` stoją w bramce danych (plan A/B §12.1). Obwiednia jest WARUNKIEM WSTĘPNYM kryterium pozostania w pracy, nie marginesem (O-29). Kopia frontowa `frtEnvelopeValidator.ts` skasowana w AB-1a (Pakiet D, w toku); obwiednia toru stabilności (`voltage_trajectory.py`) i cały tor T1 — kasacja w AB-1c po migracji ścieżek użytkownika (O-25, O-44), do tego czasu werdykty toru → `NIE_OCENIONO` (Pakiet 0). Commity: plan `dc9c55f3`/`fe5b0fef`; kod — commit integracyjny AB-1a (w toku). |

Uzupełnienie OD-18 (odbiór fali 3 W3, 2026-09-16): (c) `application/solvers/short_circuit_binding.py::execute_short_circuit`
— 0 konsumentów produkcyjnych, 5 harnessów testowych parytetu (`tests/golden/**`); jego typ wyniku `ShortCircuitBindingResult`
konsumuje chroniony `application/result_mapping/short_circuit_to_resultset_v1.py` (`PROTECTED_FILES`) — ta sama zgoda B-01 co
(a)/(b): kasacja albo przeniesienie punktu wejścia pod `backend/tests/` po decyzji właściciela, nie jednostronnie.
...
| **W5** | **Model fazowy i uziemienie** (= CV-5) | `PhaseSet` na terminalu, `Load` fazowy, `EarthingSystem`/`NeutralGrounding` jedna reprezentacja (6 → 1), `meta.field_specs` → typowany `Bay`, rozpływ niesymetryczny dla projektu, TT/IT/RCD, kasacja `EarthingSystemSelector`/`fault-loop/compute`, stan łączeniowy jedna prawda; + aneks 3a: pełna zmiana stanu łącznika i stany łączników w scenariuszu (C2/C4/C8), uziemienie ekranu kabla (F9), walidacja `vector_group` F-4 (G6) | 1, 7 | W1 | fundament pod G01, nN i W4 | CV-5 — roadmapa §4 w. 8; **karta architekta (2026-09-16): `KARTA_W5_MODEL_FAZOWY_I_UZIEMIENIE_2026-09.md`** (§1 p. 1–12, sub-karty W5-A…W5-T + `SZABLONY-NN`; inwentarz `../audit/INWENTARZ_W5_2026-09-16.md`; OD-24/OD-25 w §7); **W5-A wykonana 2026-09-16** (§1 p. 1–4; worktree z bazy `5fbe6d9a`, commity bez push, odbiór architekta; dowody `../evidence/CONVERGENCE_EVIDENCE.md` §F, rejestr `../v12xx/REJESTR_KONFLIKTOW.md` wiersz W5-A, stan decyzji `../architecture/CANONICAL_TWIN_ARCHITECTURE.md` A.4) |
...
**Aktualizacja 2026-09-16 (synteza domknięcia — `SYNTEZA_DOMKNIECIA_PRODUKTU_2026-09.md` §5, wiążąca):** kolejność W1 → W2 → W3 (odbiór fali 3) → **W6-0** (bezpieczniki S-1…S-5 natychmiast, bez B-01) → W5 → W4 → **W6-1/W6-2** (fundament czasu, równolegle z W5) → W8 → W6-3…W6-8 → W7 → W9 → W10 → W11 → W12; wiersz W6 powyżej dostaje podział W6-0…W6-8 z warunkami wejścia/wyjścia (synteza §2.11); nowe karty poza numeracją: `SZABLONY-NN` (→ W5), `KATALOG-NIEZMIENNIKI` (po W3), `ARCHIWUM-DYNAMIKA` (W6-1); decyzje OD-20…OD-23 w §7.
...
| komentarze: `network_model/core/branch.py:307`, `application/analysis_run/result_invalidator.py:14` | odsyłacze do wizarda | aktualizacja treści — **ZAMKNIĘTE dla drugiej pozycji (2026-09-17, karta KASACJA-UNIEWAZNIACZA):** moduł unieważniacza nie istnieje, więc nie ma komentarza do aktualizacji; zszedł procedurą kasacji (0 wołających w `backend/src`), bramka wskrzeszenia w `scripts/legacy_public_path_guard.py`, wpis `V12T-020` w `../v12xx/REJESTR_DLUGU.md` |
...
`test_sld_substrate_power_flow.py` z jawnymi portami końcówek); (5) fala 2 (W3-F, W3-B, W3-I, W3-C1, W3-C2) odebrana 2026-09-10 — meldunek §4 poniżej.

### §4 Meldunek wykonania W3 — fala 2: W3-F / W3-B / W3-I / W3-C1 / W3-C2 (2026-09-10) — UCZCIWOŚĆ

**Wykonane (agenci w worktree z bazy `a16f8d2b`; odbiór Fable: 26 commitów scalonych na `a1b40e9a` w worktree
`fable-w3f2`, uzgodnienie `31ccf65a`):** W3-F — skalowanie jednostek w jednym miejscu (`pochodne/jednostki.py`, 20 funkcji),
rodzina `J_skalowanie_jednostek` guarda, 153 wzorce w 54 plikach bit w bit, częstotliwość studium wątkowana do B = 2πfC
(aneks E5); W3-B — ALF ×2 → jądro `ct_burden_saturation` (`ct.alf` deleguje, `ct.alf_katalogowy` = informacja), obwód wtórny
CT/VT w modelu ENM + operacja + kreator + ekran bilansu (wiersze 1 #8, 4 #3, 4 #5); W3-I — jeden predykat wymagalności
katalogu (`wymagalnosc_katalogu`, 8 konsumentów, nowy kod W010; aneks J8/K8, §3a.M pkt 5, §4 K-A); W3-C1 — kasacja V12K-189
(20 plików), trasy JSON nastaw + ekran koordynacji na kanonie Hoppla + dobór aparatu (wiersze 4 #2, 4 #6, 10 #4); W3-C2 —
kasacja FIX-12D (2722 linie), silnik Hoppla + generacja lokalna + okno nastaw, wzorzec referencyjny na kanonie (4 #2).
Szczegóły, liczby przebiegów agentów, defekty integracji i korekty: `KARTA_W3_KONWERGENCJA_FIZYKI_2026-09.md` §5 (odbiór
fali 2); dowody pełnego łańcucha na drzewie scalonym: evidence §F („W3 fala 2 — dowody").

**Nazwane, nie ukryte:** (1) DoD W3 §4.1 doprecyzowane: zapadka J ma JEDEN trwały wyjątek (`short_circuit_to_resultset_v1.py`,
PROTECTED_FILES, B-01) — nigdy nie trafi do `pochodne/`; (2) `Branch.resolve_electrical_params` (0 wołających) i brak trasy
zapisu częstotliwości projekt → ENM — osobne karty (aneks E5); (3) `add_generator_sn` bez wczesnej bramy API/FE mimo
wymogu domeny — osobna karta (K8); (4) trzy defekty INTEGRACJI kart zielonych w izolacji, naprawione przy odbiorze (evidence
§G): helpery testów W3-C1 bez pól wymaganych przez W3-C2, literały jednostek w nowym kodzie W3-B/W3-C2 wobec rodziny J
W3-F, fixtury nN 16/17 regenerowane niezależnie przez W3-B i W3-I; (5) rejestr konfliktów: wpisy W3-C1 i W3-C2 dopisane
przy odbiorze (agenci nie pisali do współdzielonego rejestru); (6) W3-H zamknięte wariantem B (badanie śladu z dowodem → OD-15(g), stan zerowy `RegulacjaOze.tsx`; odbiór `" + D['w3h_sha'] + "`), W3-G1–G3 (fala 3, meldunki złożone) i karta TRACE-V2 — odbiór po pushu fali 2; (7) **E2E-FULL-FIX-3 (2026-09-10, `" + D['fix_sha'] + "`)** — CI Frontend E2E full na `72f867d1` czerwony (2/417: scena `macierz`, sekcja W3-D „Zgodność przekrojowa" bez atrapy → realny backend 404 → „Nie udało się…"); naprawa KLASY, nie instancji: bramka zero odpowiedzi 4xx/5xx z realnego backendu we wszystkich scenach harnessu (`e2e/nieudaneZadaniaApi.ts`, 10 testów w 3 specach pokrywających 43 sceny), atrapy końcówek czytających stan przypadku liczone Z BACKENDU (`backend/scripts/eksport_fixtur_harnessu.py` + `tests/ci/test_fixtury_harnessu.py` — zgodność przekrojowa checkerem, werdykt projektowy agregatem), bieg NC RfG/PTPiREE sceny `macierz` do REALNEGO solvera (kasacja ręcznej atrapy z klasą modułu „B" dla 0,8 MW przy 0,4 kV — progi OD-5 dają A; zasiew przeseedowany na 1,5 MW / 1,9 MW przy 15 kV), scena `stacja` buduje REALNY przypadek (projekt → przypadek → GPZ → odcinek) zamiast fikcyjnego `case-demo` (werdykt walidatora w nagłówku kroku pól był 404 „Przypadek nie należy do żadnego projektu"), konfiguracja zabezpieczeń przypadku i werdykt projektowy z atrap 1:1 z kontraktami; PRZY OKAZJI (regresja fali 2 niewidoczna dla łańcucha przedpushowego bez speców harnessu): atrapa doboru przekładników sprzed W3-B bez `kody_gotowosci`/`slad` wywracała scenę `wiazania` na biały ekran — atrapa uzupełniona, bramka kształtu sekcji sprawdza pola W3-B (nazwany błąd zamiast białego ekranu), test klasy; łańcuch przedpushowy rozszerzony o specy harnessu (`wszystkie-sceny-screenshot`, `creator-screenshot`) — evidence §A/§F/§G.

### §5 Meldunek wykonania W3 — fala 3: TRACE-V2 / W3-G3 / W3-G2 / W3-G1 (2026-09-10/16) — UCZCIWOŚĆ

**Wykonane (agenci w worktree z bazy `beaa59bb`; odbiór Fable w `fable-f3`: 14 commitów scalonych na `b89c13b3` + odbiór `c307e95f`):**
TRACE-V2 — kasacja martwego klastra „ślad v2" (backend 13 plików + 5 testów + guard CI + typy FE + dokumenty; 0 konsumentów) z bramką
wskrzeszenia obejmującą pełny inwentarz 17 klas (ślad kanoniczny = `white_box_trace` + `proof_engine`); W3-G3 — pasmo MIN/MAX
zwarcia z jednym doborem pary biegów i odmowami nazwanymi („SC 4 typy min/max" domeny 3 ma PARĘ na ekranie, nie dwa biegi do ręcznego
zestawienia); W3-G2 — pasma zdrowego rozsądku rozpływu (wiarygodność napięć/obciążeń/strat jako oś NIEZALEŻNA od zgodności); W3-G1 —
metoda rozpływu NR/GS/FD jako jawna opcja biegu z tożsamością metody w kopercie i śladzie oraz walidacją krzyżową NR↔FD (rodzina D
aneksu 3a: walidacja krzyżowa metod DZIAŁA end-to-end na jednym ekranie). Szczegóły, liczby, recenzje i naprawy przy odbiorze:
`KARTA_W3_KONWERGENCJA_FIZYKI_2026-09.md` §5 „Odbiór fali 3"; dowody łańcucha: evidence §F.

**Nazwane, nie ukryte:** (1) klasa progów napięciowych (≥6 definicji backend+frontend, jedna z błędną podstawą normową, jedna fabrykowana
krzywa spadku) → karta **W3-J** (agent, w toku); (2) defekt solvera NR (zdegenerowane rozwiązanie U ≈ 1e-17 kV, `converged=true`) →
**OD-19**; (3) `execute_short_circuit` bez konsumentów produkcyjnych w klastrze chronionym B-01 → addendum **OD-18**; (4) czerwone CI na
`b89c13b3` (parytet fixtur między maszynami, nawigacja e2e) naprawione klasą, nie instancją (karta W3 §5, evidence §G).
