...
| G16 | bieg czasowy RMS (DAE): maszyna synchroniczna + przekształtnik GFL | PARTIAL | ENM | REGRESSION_ONLY | — | `tests.golden.enm_builders.dynamika_rms:build_dynamika_rms_enm` | solver dynamiki, adapter biegu czasowego, kontrakt wyniku czasowego |
| G17 | scenariusz odniesienia SO-1A: instalacja PV 2,75 MW + magazyn energii | PARTIAL | ENM | REGRESSION_ONLY | — | `tests.golden.enm_builders.so1a_pv_magazyn:build_so1a_pv_magazyn_enm` | bramka akceptacyjna SO-1A, solver dynamiki, adapter biegu czasowego |
