...
| Specyfikacje e2e Playwright | 62 (korekta nadzorcy przy odbiorze; pierwotny pomiar 73 nie odtwarza się: `ls frontend/e2e/*.spec.ts` = 62) | `ls frontend/e2e/*.spec.ts` |
...
| U2-3 | Ten łańcuch jest BRAMKĄ — biegnie w CI przy każdym push/PR | `.github/workflows/frontend-e2e-smoke.yml` uruchamia **1 z 62** specyfikacji (korekta nadzorcy przy odbiorze: workflow wola `npm run test:e2e:real` = wylacznie `e2e/critical-run-flow.spec.ts`; spec `kd11-tozsamosc-etykiet` NIE jest w zadnym workflow): `e2e/critical-run-flow.spec.ts` (tytuł: „case -> GPZ -> trunk -> station -> branch -> katalogi -> readiness -> run -> wyniki -> SLD -> uzasadnienie -> geometria bez zmian" — **bez kroku DER**, `grep` na `add_converter_source`/`generator` w tym pliku = 0 trafień). Specyfikacje z U2-2 nie są w żadnym workflow | **NIESPEŁNIONY** — łańcuch GPZ→DER przechodzi tylko lokalnie; regresja tego łańcucha jest niewykrywalna w CI |
...
| **G-11** | Zbudować regresję wizualną nowej powłoki na bazie odniesienia (dziś `toHaveScreenshot` nie występuje w żadnej z 62 specyfikacji, a konfiguracja tolerancji w `playwright.config.ts` jest martwa) | polish (E1–E8) | U5 | CI + e2e | **L** | G-10 (wspólna infrastruktura biegu e2e w CI) |
