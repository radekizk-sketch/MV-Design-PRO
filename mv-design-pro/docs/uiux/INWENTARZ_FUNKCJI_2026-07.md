...
| Rozpływ niesymetryczny | S6 | ❌ (2026-09-09, karta K2: `POST /api/v1/reference-networks/{id}/run`+`/validate` skasowane razem z całym dawnym dialektem `application/reference_networks/**`) | ❌ (2026-09-09, karta K2: `ui/reference-networks/ReferenceNetworkRunPanel`, powierzchnia E-39 i karta „Walidacja sieci referencyjnych" huba `ui2/wyniki/analizy/model.ts` skasowane w całości — decyzja architekta: walidacja solverów vs benchmarki jest zdolnością weryfikacyjną, nie tokiem pracy projektanta) | ❌ (2026-09-09, K2: ROUTERY-4A [2026-08-07] ❌→◐ COFNIĘTE do ❌ — `power_flow_unbalanced.py` (FROZEN, B-01, NIETKNIĘTY) ma dziś WYŁĄCZNIE konsumentów TESTOWYCH — jednostkowy `backend/tests/test_power_flow_unbalanced.py` (sprzed karty K2, nietknięty, fikstury syntetyczne 2-szynowe) i nowy regresyjny `backend/tests/golden/test_solver_unbalanced_bfs_ieee34.py` [regresja na publikowanej topologii IEEE 34-bus, REGRESSION_ONLY, bez niezależnej wyroczni fizyki niesymetrycznej] — ZERO konsumenta PRODUKCYJNEGO od karty K2 (poprzedni jedyny, `application/reference_networks/computation.py`, skasowany w całości). ZERO ścieżki UI do tego solvera — bieg na modelu PROJEKTU nadal nie istnieje też. Mapa domknięcia 3 #2: W5) |
