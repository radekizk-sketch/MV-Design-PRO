...
| Stabilność SSCI | A18 | ⊘ WYCOFANA (2026-09-23, AB-1d_min): `POST /api/cases/{id}/runs/v126/ssci_impedance` i `GET …/ssci_impedance/stability` → 410 (rejestr zdolności `withdrawn`); kod badawczy `application/analyses/ssci_stability` zostaje pod testem do AB-5H | okno `ui2/wyniki/ssci` i zakładka „Stabilność SSCI" USUNIĘTE; katalog „Analizy specjalistyczne" pokazuje kartę wycofania (powód + warunek powrotu) | ⊘ (2026-09-23: werdykt stał na zapasie fazy 30° zaszytym w kodzie i Z_grid(f) z impedancji źródła przyjętej z założenia — przegląd adwersarialny §6.4; historia: 2026-08-06 UI domknięte) |
