...
**Rewizja punktowa:** 2026-09-05 (karta CV-3.3-B2) — wiersz A16 (§6, „Porównania scenariuszy"): `protection_comparisons` OSIĄGALNE — jeden ekran `ui2/wyniki/porownanie` z przełącznikiem rodzaju rozpływ/zwarcia/zabezpieczenia; martwe strony `ui/{power-flow,protection}-comparison/*Page.tsx` skasowane. Generyczne `/api/comparison/runs` (P10b) pozostaje bez konsumenta (poza zakresem karty).
...
| S20 | Spadek napięcia na kablu (preview) | Preview spadku napięcia kabla | `cable_voltage_drop.py` (endpoint `/api/solver/cable-voltage-drop-preview`, `/api/solver/cable-rated-current-preview`; ocena doboru przekroju odcinka i ciągu magistrali SN rekordami werdyktu wyjaśnialnego: `/api/solver/trunk-sizing-assessment`, `application/analyses/ocena_doboru_magistrali.py`, suma spadków ciągu `compute_trunk_voltage_drop`); korekta obciążalności: `cable_ampacity_derating.py` (endpoint `/api/solver/cable-ampacity-derating-preview`, `/api/solver/cable-laying-conditions`) |
...
| S22 | Dynamika RMS (DAE) | Bieg czasowy RMS od punktu pracy rozpływu: rdzeń DAE (trapez/RK4), urządzenia (maszyna, GFL, GFM, magazyn, turbina), zdarzenia (zwarcia w węźle i w linii x·L z predykatem izolacji, łączenia gałęzi i odsprzęgów, odbiory, obszary beznapięciowe, przypisania stanu i komendy regulacji P/Q/U, częściowa utrata źródła, profil U/f/θ źródła testowego w trybie stanowiska, zdarzenia warunkowe z detektorami przekroczeń), obserwable (częstotliwość węzła z kodem jakości, fazory prądów obu zacisków gałęzi), próbki obustronne `L`/`P` w chwilach zdarzeń | `dynamika/` (pakiet; adapter `enm/adapter_dynamiki.py`, wykonawca `enm/canonical_analysis.py::_execute_dynamika_rms`, kontrakt `application/contracts/resultset_dynamic_v2.py`) |
...
## 4. Powierzchnia API (`backend/src/api/`, 61 modułów — pomiar 2026-09-09, karta K2: 62→61,
`api/reference_networks.py` skasowany)
...
power_flow_comparisons, power_flow_runs, ~~reference_networks~~ (skasowany K2 2026-09-09), project_archive, projects, proof_pack,
protection_comparisons, protection_analysis_runs, reference_patterns, ~~sld~~ (skasowany W1 2026-09-09), station_templates,
...
- **USUNIĘTE W1 (2026-09-09)** (legacy persystencja sieci — jedna prawda sieci to ENM): router `sld`
  ...
  (`GET|POST /api/catalog/export|import`; jedyny konsument — przyciski eksportu/importu w
  `ui/catalog/TypeLibraryBrowser.tsx` — skasowany razem z nimi) oraz `type-ref`/`equipment-type`
  w `catalog` (bez konsumenta w `frontend/src`, pomiar 2026-09-09); bramka wskrzeszenia w
  `legacy_public_path_guard.py`.
- **ŚWIADOMIE ODSTAWIONE** (moduł zostaje, router niewpięty, decyzja o ZDOLNOŚCI należy do
  ...
  cloud_backup (decyzja o powierzeniu danych zewnętrznemu dostawcy). protection_coordination
  zamontowany 2026-08-13 (ZAB-100-BACKEND); archive_diff i incremental_archive zamontowane
  2026-09-24 (karta ARCHIWUM PROJEKTU) z konsumentem — okno „Archiwum projektu (ZIP)".
...
projects, proof, property-grid, protection, protection-comparison,
protection-coordination, protection-curves, ~~reference-networks~~ (skasowany K2 2026-09-09), reference-patterns, reports,
...
> Zmiana od 2026-09-09 (W1): usunięto katalog `project-archive` (55 → 54) — dialog
> warstwy zastanej bez konsumenta produkcyjnego (G-07), czytał pola podglądu archiwum
> 2.x, których format 3.0.0 nie niesie.
...
| Rozpływ niesymetryczny | S6 | ✅ (2026-09-16, karta W5-D: rodzaj wykonawczy `PF_UNBALANCED` w `POST /api/execution/study-cases/{id}/runs` → `enm/canonical_analysis.py::_execute_power_flow_unbalanced` (jedyny dyspozytor `_wykonaj_analize_biegu`), wejście przez JEDEN assembler `enm/assembler.py::zloz_wejscie_rozplywu_niesymetrycznego` (ENM → IR → `UnbalancedNetworkInput`, Z_s/Z_m ze składowych w `network_model/pochodne/skladowe_symetryczne.py`, analiza drogi I0 odbiorów faza–N `DrogaZerowa`), kontrakt `domain/result_contract_power_flow_unbalanced_v1.py`, końcówka `GET /api/analysis-runs/{id}/results/rozplyw-niesymetryczny`, gotowość `_check_asymmetry` na tej samej diagnozie; odmowy nazwane kodami kanonu: `power_flow.unbalanced_requires_radial`, `…_load_phases_unsupported`, `…_element_unsupported`, `…_no_zero_sequence_path`, `branch.zero_sequence_missing` [bez podstawiania Z0 = Z1], `transformer.vector_group_missing`; założenia biegu nazwane: `…_transformer_series_model`, `…_magnetising_branch_omitted`, `…_shunt_admittance_omitted`, `…_losses_self_impedance`, `…_zero_sequence_confined`, `…_zero_sequence_via_source`) | `ui2/spaces/obliczenia/UruchomObliczenie` (rodzaj „Rozpływ niesymetryczny") + `ui2/wyniki/stan-fazowy/EkranStanuFazowego` (E-31, DRUGIE źródło: szyny per faza + VUF z solvera, gałęzie per faza, podsumowanie, założenia biegu) + kreator `ui2/kreatory/odbior` (pole `Load.phases`, zerowa wartość domyślna); model: `Load.phases: PhaseSet | None` (addytywne, poza hashem gdy puste — 56 sieci rejestru bit w bit); e2e `e2e/rozplyw-niesymetryczny.spec.ts` (bramka CI `frontend-e2e-smoke`) | ✅ (2026-09-16, W5-D: bieg na modelu PROJEKTU istnieje end-to-end; solver FROZEN `power_flow_unbalanced.py` NIETKNIĘTY — konsumenci produkcyjni: `enm/assembler.py`, `enm/canonical_analysis.py`, `enm/rozplyw_niesymetryczny_wynik.py`; parytet IEEE 34 z wyrocznią `ieee_34bus.json` przez bieg kanoniczny [max|ΔU| 2,83e-4 pu, zero xfail]; sieć symetryczna = NR [U ≤ 1e-6 pu, I ≤ 0,25 %, straty ≤ 0,5 %]) |
| Falowniki OZE w LF | S7 | generators, grid_source_preview, operacje ENM | `ui2/kreatory/zrodlo-oze` — regulacja falownika: `control_mode` STALY_COS_PHI / Q_OD_U (spadek + strefa nieczułości) / **REGULACJA_NAPIECIA** (nastawa U, węzeł PV — karta CV-4.1b, A3-04) / WYLACZONE, q_min/max, magazyn; REGULACJA_NAPIECIA bramkowana profilem operatora NC RfG (`voltage_control_modes`); 6+11 testów | ◐ (kontrakt `PowerFlowResult`/`PowerFlowTrace` nie niesie wyniku regulacji STALY_COS_PHI/Q_OD_U — w wynikach nie widać, który z tych dwóch trybów zadziałał ani czy limit Q został osiągnięty; REGULACJA_NAPIECIA WYJĄTKIEM: `pv_to_pq_switches` + moduł U i moc bierna Q węzła PV w wyniku FROZEN już niosą to rozstrzygnięcie, bez zmiany kontraktu) |
| Obciążenia ZIP | S8 | solver_input (materializacja katalogowa), `add_nn_load` | `ui2/kreatory/odbior` — sekcja „Model obciążenia (ZIP)": udziały `a`/`b`/`c` dla P i Q, wrażliwość częstotliwościowa `k_pf`/`k_qf`, sumy na bieżąco, walidacja `a+b+c = 1`; prefill z pozycji katalogowej; 31 testów kreatora + 23 kontraktu backendu | ✅ (2026-08-06: komplet ośmiu współczynników dopisany do `ui_fields` kontraktu OBCIAZENIE i wystawiony w kreatorze; wartości płyną do `Load.materialized_params`, skąd czyta je rozpływ. Wejście do modelu ma JEDEN kontrakt — `enm/load_zip_model.py` — wołany przez wszystkie drogi zapisu: `add_nn_load`, `add_load_sn`, `create_device`, `update_element_parameters` i krok K6 kreatora sieci (`application/network_wizard`) — korekta 2026-09-25: dawne „trzy drogi" pomijało `add_load_sn` i kreator K6, który nie walidował tabliczki wcale. Stała moc nie zapisuje żadnego klucza, więc istniejące projekty liczą identycznie. Od karty modeli odbiorów (AB-1b.3a, O-49) pole `Load.model` jest WYPROWADZANE ze współczynników (`load_zip_model.model_odbioru`) i nikt go nie czyta w rachunku — rozpływ, dynamika i projekcja v2 czytają jeden predykat `jest_odbiorem_zip`; odniesienie `f0` bez wartości = częstotliwość studium, a szyna, której odbiorów ZIP rozpływ nie odwzorowuje dokładnie, jest odmową `load.zip_agregat_niereprezentowalny`) |
...
| Zabezpieczenia IEC 60255 | S10 + A10, A11 | protection_* (5 routerów); zacisk zabezpieczenia (2026-09-23, AB-1b.1a, O-51): resolver `application/protection_settings/zacisk_zabezpieczenia.py` (reguła szeregowa KCL), `GET /api/cases/{case_id}/enm/zacisk-lokalizacji`, parametr `zacisk_zabezpieczenia` tras nastaw | `ui2/wyniki/koordynacja` (EkranKoordynacji + SekcjaNastaw z wyborem zacisku i powodem blokady, 2 testy), `ui/protection-coordination` (edytor urządzenia: zacisk gałęzi z etykietami backendu; prąd roboczy i zwarciowy zacisku), `ui2/model/zabezpieczenia-automatyka` (E-27, test) | ✅ |
...
| Dynamika RMS (DAE) | S22 | ✅ execution_runs (DYNAMIKA_RMS) → `enm/canonical_analysis.py::_execute_dynamika_rms`; `GET /api/analysis-runs/{id}/results/dynamika` (metadane, kontrakt `resultset_dynamic_v2`) i `.../results/dynamika/time-series` (próbki z `strona_probki`, `None` = wartość niedostępna) | BRAK ekranu (tor T2 bez powierzchni UI — `docs/audit/CURRENT_DYNAMIC_CAPABILITY_MATRIX.md` §3) | ❌ (2026-09-23: backend, kontrakt i testy ścieżki HTTP `tests/test_dynamika_rms_run.py`; 2026-09-24 AB-1b.1b: komenda regulacji, częściowa utrata, stanowisko badawcze i detektory przez tę samą ścieżkę HTTP — pola wyniku `przekroczenia`, `tryb_scenariusza`, `przypisania`; front nie woła końcówek) |
...
| Stan fazowy SN | S15 | ✅ analysis_runs + execution_runs | `ui2/wyniki/stan-fazowy/EkranStanuFazowego` (E-31: napięcia/prądy/straty per faza, wskaźniki asymetrii z flag solvera, stan obwodu, ograniczenia raportowe), zakładka „Stan fazowy"; od W5-D (2026-09-16) ekran czyta też DRUGIE źródło — bieg `PF_UNBALANCED` (`GET …/results/rozplyw-niesymetryczny`: szyny per faza + VUF wg składowych symetrycznych z solvera BFS, gałęzie per faza, założenia biegu), nazwa źródła zawsze na ekranie, stan wskaźnika per źródło (odchylenie od średniej faz — solver stanu fazowego; VUF — rozpływ niesymetryczny); test | ✅ (2026-08-06: poprzednie ◐ „9 plików" pochodziło sprzed powstania ekranu) |
...
| Porównania scenariuszy | A16 | comparison (`/api/comparison/runs`), power_flow_comparisons, protection_comparisons, `/api/short-circuit-comparisons` | `ui2/wyniki/porownanie` — przełącznik RODZAJU (rozpływ: szyny/gałęzie/ranking; zwarcia; **zabezpieczenia: zmiany stanu urządzeń/ranking/ślad porównania White Box**), ten sam mechanizm wyboru A/B i ten sam panel proweniencji w każdym trybie, z deep-linkiem do dowodu przebiegu A i B; 8 plików testów | ◐ (2026-09-05, CV-3.3-B2: `protection_comparisons` OSIĄGALNE — martwe strony `ui/{power-flow,protection}-comparison/*Page.tsx` skasowane, tryb „Zabezpieczenia" ekranu ui2 czyta `GET /projects/{id}/protection-runs` + `POST /api/protection-comparisons` + `GET .../trace`, e2e z realnym backendem: dwa biegi zabezpieczeń na różniących się modelach → tabela ekranu = pole z odpowiedzi backendu. Pozostaje: generyczne `/api/comparison/runs` (P10b) nadal BEZ ŻADNEGO konsumenta produkcyjnego — poza zakresem tej karty) |
...
| Kontyngencje N-1 (enumeracja) | `application/analyses/kontyngencje_n1.py` (ORKIESTRACJA istniejącego solvera rozpływu: wariant wejścia bez elementu → bieg → macierz skutków; kryteria z walidacji energetycznej A4 i z wyspy węzła bilansującego solvera) | ✅ `GET /api/insights/n-1-contingency?run_id=[&element_refs=]` + `GET /api/insights/n-1-contingency/scope?run_id=` (zapowiedź zakresu: lista kwalifikowanych elementów + koszt biegu liczbami, bez biegu solvera) — `api/analysis_insights.py` | ✅ Okno „Kontyngencje N-1" (`ui2/wyniki/kontyngencje`, zakładka warsztatu Wyników): zakres wybierany przez inżyniera (komplet albo wskazane elementy), koszt przed startem, przypadek bazowy N-0 zawsze nad rankingiem, ranking dotkliwości, skutki per wyłączenie, `kryteria_pominiete` i `nierozstrzygniete` jawnie | ✅ (2026-08-14, karta EKRAN-N1: łańcuch domknięty end-to-end — luka O.1 audytu zamknięta po OBU stronach. Dług wydajności (2,64 s na kontyngencję) domknięty NIE optymalizacją, lecz decyzją inżyniera: zawężenie zakresu z listy podanej przez backend albo bieg pełny ze ZNANYM rozmiarem (liczba kontyngencji i liczba biegów solvera; czasu ekran nie podaje, bo kontrakt nie niesie pomiaru dla bieżącego modelu). Skracanie listy heurystyką pozostaje zakazane — przypięte testem „Zaznacz wszystkie bierze KOMPLET") |
...
| Import XLSX | — | ✅ xlsx_import: `POST /api/import/xlsx/preview` (podgląd bez zapisu) + `POST /api/import/xlsx` (nowy projekt: arkusz → `SiecZArkusza` → kompilator grafu `enm/kompilator_grafu.py` → ENM projektu + katalog projektu `PROJEKTOWY_V1` z proweniencją wiersza, odpowiedź z `enm_hash`, transakcyjnie — W1 (2026-09-09); wcześniej zapis wyłącznie do legacy ORM) | `ui2/spaces/projekt/arkusz` — okno „Import sieci z arkusza (XLSX)": kafel pulpitu, podgląd „co wejdzie do modelu" z backendu, raport zastrzeżeń per wiersz (arkusz·wiersz·kolumna), bramka katalogowa, jawny następny krok; 13 testów natywną ścieżką | ✅ (karta XLSX-IMPORT 2026-08-07: teza „końcówka działa, brak konsumenta" OBALONA pomiarem — końcówka zwracała 422 „Brak biblioteki openpyxl" dla KAŻDEGO wejścia (biblioteki nie było w zależnościach), a wstrzykiwany `uow_factory` nie był używany: import niczego nie zapisywał, tylko meldował sukces. Naprawa u źródła: zależność, zapis do kanonicznego magazynu modelu, zero fizyki w warstwie aplikacji (dane źródła jako wejście), zero wartości fikcyjnych (`rated_current_a=1.0` usunięte), bramka katalogowa wspólna z importem ZIP) |
| Archiwum projektu (ZIP) | — | ✅ project_archive: `POST /projects/{id}/export` (parametr `zapisz_do_magazynu` → magazyn dokumentów, typ ARCHIWUM), `POST /projects/import`, `POST /projects/import/preview` — archiwum wersji 3.0.0 wyłącznie z ENM (sekcje legacy `network_model`/`sld_diagrams`/`proofs` usunięte, import 2.x przez kompilator grafu; W1 (2026-09-09)); archive_diff: `POST /archives/diff` (dwa pliki) + `POST /archives/diff/projects/{a}/{b}` (dwa projekty) — wynik po nazwach elementów i etykietach PL; incremental_archive (bez stanu w procesie, 2026-09-24): `POST /projects/{id}/export/incremental` (paczka zmian względem przesłanej paczki bazowej) + `POST /projects/import/incremental` (paczka bazowa + paczka zmian → NOWY projekt drogą importu pełnego); błędy archiwum nazwane ze ścieżką pola → 422, paczka innej bazy → 409 | ui2/spaces/projekt/archiwum (okno „Archiwum projektu (ZIP)": kafel pulpitu + karta huba dokumentacji; sekcje: spakuj / odtwórz / porównaj wersje / przekaż same zmiany); `ui/project-archive` (warstwa zastana bez konsumenta) SKASOWANY w W1 2026-09-09; podgląd archiwum 3.0.0 pokazuje warianty/biegi kanoniczne/zapisy modelu ENM | ✅ (2026-09-24, karta ARCHIWUM PROJEKTU: porównanie i paczka zmian wpięte z ekranu do backendu; e2e na realnym backendzie `e2e/archiwum-porownanie-wersji.spec.ts`; wcześniej ◐ — G-01 audytu bramek U2–U5) |
| Katalog typów | — | catalog (46 końcówek — pomiar 2026-08-14; W1 (2026-09-09): eksport/import biblioteki typów, `type-ref` i `equipment-type` skasowane razem z tabelami typów w bazie — katalog statyczny + katalog projektu w ENM; w tym `GET /api/catalog/switchgear-families` i subzasób `factory-configurations`: 18 rodzin rozdzielnic SN/RMU, 95 konfiguracji fabrycznych, walidator `family_supports`), audit2_catalogs | `ui2/spaces/model/katalog` (KatalogPanel, KartaTechniczna, GdzieUzyty, ParametrRow); 7 testów; rodziny+bloki konsumowane przez krok 4 kreatora stacji (tory MODULARNY/BLOK_RMU) | ✅ |
| Kreator sieci/stacji | — | station_templates, switchgear_config, reference_patterns; operacje kanoniczne ENM `add_sn_bay` / `add_sn_bay_from_catalog` (dry_run = źródło werdyktu podglądu pola; zapis pól stacji idzie operacjami stacyjnymi czytającymi TEN SAM wybór bloku fabrycznego — karta K-M 2026-08-14); design_synth NIEWPIĘTY | `ui2/kreatory` — 18 kreatorów, KAŻDY z testem (48 plików testowych) + `ui2/spaces/model/szablony` (przeglądarka i porównanie szablonów stacji) | ◐ (`/api/reference-patterns` ma jedynego konsumenta w `ui/reference-patterns`, bez montażu produkcyjnego, a router `design_synth` nadal nie ma `include_router` w `main.py`) |
...
| SLD + nakładki wyników | — | sld_overrides (router `sld` skasowany W1 (2026-09-09) — diagramy w bazie bez konsumenta; nakładka zabezpieczeń w `protection_runs`) | `ui2/spaces/schemat` (przełącznik podglądu + następny krok, 2 testy) osadzający zastane `ui/sld`, `ui/sld-editor`, `ui/sld-overlay` | ◐ (OSOBNY WĄTEK — patrz Program §2.3) |
...
nadal nie mają powierzchni (domknięte 2026-09-24 kartą ARCHIWUM PROJEKTU — wiersz ✅). Pełna rewizja pozostałych wierszy — wykonana
...
**Aktualizacja 2026-09-09 (karta K2, CV-4.3 — decyzja architekta, druga ścieżka fizyki
usunięta).** `application/reference_networks/**` (dawny dialekt benchmarków, druga ścieżka
budowy wejścia solwerów równoległa do kanonicznej ENM), `api/reference_networks.py` (9 tras)
i ekran E-39 (`ReferenceNetworkSurface`, karta „Walidacja sieci referencyjnych" huba
`ui2/wyniki/analizy/model.ts`) skasowane w całości — walidacja solverów vs publikowane
benchmarki jest zdolnością WERYFIKACYJNĄ (CI, `tests/golden/enm_builders/**` +
`tests/golden/registry.py`), nie tokiem pracy projektanta. **Rozpływ niesymetryczny COFNIĘTY
◐→❌** (ROUTERY-4A 2026-08-07 domknęło ten wiersz WYŁĄCZNIE przez `runReferenceNetwork`/E-39,
usunięte tą kartą razem z resztą dialektu — nie istniała inna ścieżka UI do
`power_flow_unbalanced.py`). Solver FROZEN (B-01) NIETKNIĘTY, ma dziś wyłącznie konsumenta
TESTOWEGO (`backend/tests/golden/test_solver_unbalanced_bfs_ieee34.py`, regresja na IEEE
34-bus) — mapa domknięcia 3 #2, wycinek W5. Bilans po karcie: **27 ✅ / 17 ◐ / 1 ❌**
(jedyny ❌: Rozpływ niesymetryczny, S6 — brak drogi UI, nie brak solvera).
...
> Uzupełnienie 2026-09-16 (karta W5-D, wycinek W5): **Rozpływ niesymetryczny ❌→✅** — bieg na modelu projektu istnieje end-to-end (model `Load.phases` → kreator odbioru nN → rodzaj `PF_UNBALANCED` w przestrzeni „Obliczenia" → jeden assembler ENM→IR→`UnbalancedNetworkInput` → solver FROZEN NIETKNIĘTY → `ResultSetPowerFlowUnbalancedV1` → rejestr biegów → końcówka `rozplyw-niesymetryczny` → ekran E-31 jako drugie źródło → e2e na realnym backendzie w bramce `frontend-e2e-smoke`). Bilans po karcie: **28 ✅ / 17 ◐ / 0 ❌**. Nazwane ograniczenia solvera FROZEN (B-01, decyzja właściciela): straty gałęzi z impedancji własnej |I|²·Z_s (bez wyrazu wzajemnego — dotyczy tylko krawędzi z realnym I0 i Z0 ≠ Z1) oraz brak izolacji składowej zerowej przez trójkąt (transformator = macierz 3×3 szeregowa; krawędzie powyżej punktu zamknięcia I0 liczone z Z_m = 0) — oba emitowane jako założenia biegu z kodami kanonu.
...
| `GET /api/analysis-runs/{run_id}/nastawy?linia=&nastepna_szyna=` | `api/analysis_runs.py` (karta W3-C1) | Nastawy I>/I>> metodą Hoppela (JSON) — zastępuje usuniętą `GET /api/protection/overcurrent-settings` (V12K-189, 0 producentów/konsumentów w chwili kasacji) |
| `GET /api/analysis-runs/{run_id}/nastawy/dopasowanie?device_id=&linia=&nastepna_szyna=` | `api/analysis_runs.py` (karta W3-C1) | Dopasowanie nastaw Hoppela do aparatu z katalogu analitycznego (funkcja czysta, bez koperty biegu) |
...
| `ui2/wyniki/koordynacja/` — sekcja „Nastawy wyznaczone z analizy" PRZED stroną selektywności; kreator OZE (`ui2/kreatory/zrodlo-oze/`) dostał `cos φ toru` i `Charakter mocy biernej falownika` | `ui2/wyniki/koordynacja/{nastawyApi.ts,SekcjaNastaw.tsx}` (karta W3-C1, 2026-09; zastępuje kontrakt V12K-204 — metodyka V12K-189, którą tamten kontrakt prezentował, skasowana zero-konsumentową kartą W3-C1, jedyna metodyka nastaw nadprądowych jest odtąd Hoppel/IRiESD); `ui2/kreatory/zrodlo-oze/{derSelectionApi,zrodloOzeDobor,DoborToruSn}` (V12K-203) | Kotwica (bieg zwarcia 3F c_max) → wybór odcinka i kolejnej szyny → nastawy I>/I>>/cieplne/SPZ z pakietem dowodowym i (nowość W3-C1) doborem aparatu; przypadek pracy toru DER wybierany, a ΔU nazwane wzrostem albo spadkiem |
...
| Nastawy nadprądowe: solver ✔ / API ✖ / UI ✖ → ✔ solver + API + UI | `application/protection_settings/{engine,batch_run}.py`; `/api/analysis-runs/{run_id}/nastawy[/dopasowanie]`; `ui2/wyniki/koordynacja/SekcjaNastaw.tsx` | Dług V12K-189 (druga metodyka, 0 producentów/konsumentów) skasowany kartą W3-C1 (2026-09) zamiast domknięty na miejscu — jedyna metodyka jest Hoppel/IRiESD, dostępna z ekranu koordynacji jako JSON i pakiet dowodowy, z doborem aparatu |
...
| Delta | Dowód | Uzasadnienie |
|---|---|---|
| Enumeracja kontyngencji N-1: pozycji nie było → analiza ✔ + API ✔ + UI ✔ (2026-08-14) | `application/analyses/kontyngencje_n1.py`; `GET /api/insights/n-1-contingency` + `/scope`; `tests/application/analyses/test_kontyngencje_n1_service.py` (29), `tests/api/test_kontyngencje_n1_api.py` (11); `frontend/src/ui2/wyniki/kontyngencje` (38 testów) | Zdolność O.1 (D8). Zero nowej fizyki: dla każdego kwalifikowanego elementu (linia, kabel, transformator) powstaje wariant WEJŚCIA solvera bez tego elementu i biegnie ISTNIEJĄCY solver rozpływu; model w magazynie pozostaje nietknięty (Case Immutability). Ekran (karta EKRAN-N1) nie zna reguły kwalifikacji ani reguły wykluczenia — obie przychodzą z zapowiedzi zakresu, więc zbiór oferowany inżynierowi jest DOKŁADNIE zbiorem, który bieg policzy (równość przypięta testem po obu stronach) |
