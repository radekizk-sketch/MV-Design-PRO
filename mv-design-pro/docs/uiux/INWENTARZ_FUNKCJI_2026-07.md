...
| S22 | Dynamika RMS (DAE) | Bieg czasowy RMS od punktu pracy rozpływu: rdzeń DAE (trapez/RK4), urządzenia (maszyna, GFL, GFM, magazyn, turbina), zdarzenia (zwarcia w węźle i w linii x·L z predykatem izolacji, łączenia gałęzi i odsprzęgów, odbiory, obszary beznapięciowe), obserwable (częstotliwość węzła z kodem jakości, fazory prądów obu zacisków gałęzi), próbki obustronne `L`/`P` w chwilach zdarzeń | `dynamika/` (pakiet; adapter `enm/adapter_dynamiki.py`, wykonawca `enm/canonical_analysis.py::_execute_dynamika_rms`, kontrakt `application/contracts/resultset_dynamic_v2.py`) |
