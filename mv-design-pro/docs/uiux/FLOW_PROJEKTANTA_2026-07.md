...
| F-E6.2 pętla decyzji — rozszerzenie na kolejne ekrany + akcje kontekstowe | E6 | ⚠️ **CZĘŚCIOWO (pomiar 2026-07-28)**: akcje kontekstowe per rodzaj przekroczenia ISTNIEJĄ (`ui2/wyniki/wzorzec/akcjeNaprawcze.ts`: `RodzajPrzekroczenia` → `akcjaNaprawcza`), pętla wpięta w 15 z 18 modułów wyników — w tym zwarcia (3 trafienia) i jakość (1). **NIEPODPIĘTA POZOSTAJE KOORDYNACJA**: `ui2/wyniki/koordynacja/EkranKoordynacji.tsx` ma ZERO akcji naprawczych, a to jedyny ekran wyników z przekroczeniem typu „brak selektywności", dla którego akcja (→ nastawy zabezpieczeń) była w karcie nazwana wprost. Moduły `analizy` (hub) i `dowod` (pakiet dowodowy) słusznie bez pętli — nie mają wierszy z przekroczeniem. |
