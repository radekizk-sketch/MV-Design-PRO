...
| **FALA „POZIOM EKSPERTA" (dyrektywa właściciela 2026-07-22: „wróć do pierwotnego domknięcia flow projektanta i poprawiaj to, co nie dojechało poziomem")** — pomiar: 6 kart huba analiz (E-29…E-34) wciąż na ZASTĘPCZYM kontrakcie F-E5a mimo istnienia realnych dostawców/danych; 10 legacy formularzy kreatorów; `show-ncrfg` w legacy E-26 | E5/E7/E2 | w toku 2026-07-22 |
| P-1 przekierowania: E-33→wkłady zwarć (F2), E-34→bilans IEC + dobór aparatów, show-ncrfg→macierz ui2, tabela pozostałych akcji SLD | E5/E7 | karta wykonawcy (w locie) |
| P-2 realne ekrany: E-30 „Zbieżność rozpływu i zaczepy" + E-31 „Stan fazowy SN" (wzorzec ekranu analizy, dane kanoniczne) | E5 | karta wykonawcy (w locie) |
| P-3 realne ekrany: E-29 „Składowe symetryczne i sieć zerowa" + E-32 „Stabilność dynamiczna" | E5 | karta wykonawcy (w locie) |
| P-4/P-5 kreatory: przebudowa 10 pozostałych legacy formularzy (`ui/network-build/forms/`: Dispatchable/Relay/Measurement/NnOutgoingField/BranchPole/Zksn/StartBranch/AssignCatalog/ChooseSnSegmentFamily/UpdateElementParameters) na framework `kreatory/rama` | E2 | fala 2 (po scaleniu P-1…P-3) |
...
| F-E8.3 Magazyn wygenerowanych dokumentów (cykl życia) — BACKEND | E8 | do zlecenia (recenzja pkt 4-pełne/6-pełne/3-Podgląd/Pobierz) — persystencja wygenerowanego dokumentu: status (Gotowy/W trakcie/Błąd), liczba stron, data wygenerowania, plik do Podglądu/Pobrania. Wymaga rozbudowy backendu (osobny, przetestowany krok) — NIE fabrykować metadanych dokumentu, który nie istnieje. Karty huba czytałyby wtedy realny status zamiast „Do wygenerowania". |
...
| F-E8.3 Magazyn wygenerowanych dokumentów + zestawienia materiałowe — BACKEND | E8 | do zlecenia — (a) cykl życia wygenerowanego dokumentu (status Gotowy/W trakcie/Błąd, liczba stron, data, plik do Podglądu/Pobrania); (b) generator „Zestawienie materiałowe / BOM" (grep pusty — brak backendu). Wymaga rozbudowy backendu (osobny, przetestowany krok). NIE fabrykować metadanych/BOM na ekranie. |
...
| F-E6.2 pętla decyzji — rozszerzenie na kolejne ekrany + akcje kontekstowe | E6 | do zlecenia — wpiąć `onPoprawWModelu` w zwarcia/jakość/koordynację; akcja kontekstowa per rodzaj przekroczenia (ΔU→konfigurator odcinka, przeciążenie→dobór kabla, miscoordination→nastawy zabezpieczeń) zamiast ogólnego „przejdź do modelu". |
