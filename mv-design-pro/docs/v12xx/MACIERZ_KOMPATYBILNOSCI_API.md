...
| `GET /api/analysis-runs/{run_id}/pakiet-dowodowy` | v12xx | aktywny | 2026-08-07 | - | Pakiet dowodowy przebiegu (ZIP: dowod, zrodlo LaTeX, wykaz plikow, odcisk); rodzaj pakietu z danych biegu. Rodzaje zwarciowe przyjmuja `punkt` = punkt zwarcia (wymagany). Pakiet rozplywu jest ZBIORCZY (rozplyw + straty + spadek napiecia) i przyjmuje `punkt` = ODCINEK linii/kabla (koniec trasy), NIEOBOWIAZKOWO: brak odcinkow w biegu nie odbiera bilansu mocy, a odcinek spoza biegu = 422 (karta PACK-BEZ-KONSUMENTA, 2026-08-08). Dowod spadku napiecia jest LANCUCHEM od zrodla (SLACK) do wybranego odcinka — obejmuje wszystkie odcinki linii/kabla ORAZ granice transformatorow na trasie (SN kabel -> TR -> odcinki nN), nie tylko wskazany odcinek z osobna (karta P0.5b, 2026-08-13, likwidacja limitu MVP=1 i drugiej formuly delta-U, N-D6). | pakiet dowodowy biegu API tests | Architekt dowodow |
...
| `GET /api/quality/voltage-profile` | v12xx | aktywny | 2026-08-13 | - | Profil napieciowy z dekompozycja dU per odcinek wzdluz sciezki zrodlo->wezel (node_ref) lub najgorszej sciezki nN (worst_nn); bez parametrow zwraca widok BUS-centric bez zmian (addytywne, P0.4 modul nN). ZERO fizyki (odczyt wynikow rozplywu). | voltage profile segment tests | Architekt jakosci |
...
| `GET /api/cases/{case_id}/enm/fault-loop-feeders` | v12xx | aktywny | 2026-08-13 | - | Petla zwarcia we WSZYSTKICH punktach nN, pogrupowana per odplyw (karta P0.6, G-05); kazdy osiagalny punkt kazdego odplywu + wskazanie punktu najgorszego. Fizyke liczy solver (trasa z grafu + upstream Thevenin), koncowka jest read-only. | tests/api/test_kd9_kontrakt_koncowek_bez_testu.py, tests/application/analyses/fault_loop/test_service.py | nadzor programu |
| `GET /api/cases/{case_id}/enm/fault-loop-point` | v12xx | aktywny | 2026-08-13 | - | Petla zwarcia w DOWOLNYM punkcie nN (karta P0.6, G-05); trasa REALNA z grafu (BFS kabel po kablu, zyla powrotna PE/PEN, n_parallel) + upstream Thevenin SN. Fizyke liczy solver, koncowka jest read-only; model bez wskazanej stacji/trasy daje jawny stan `brak danych`. | tests/api/test_kd9_kontrakt_koncowek_bez_testu.py, tests/application/analyses/fault_loop/test_service.py | nadzor programu |
...
| `GET /api/cases/{case_id}/enm/nn-device-selection` | v12xx | aktywny | 2026-08-13 | - | Dobor aparatu zabezpieczajacego nN per obwod (karta P0.7, §0.5): cztery kryteria (Ib<=In<=Iz′, I2<=1,45·Iz′, zdolnosc wylaczania >= Ik″max, SWZ przy Ik_min — REUSE pakietu SWZ) ocenione dla wszystkich kandydatow katalogu (MCB/rozlacznik bezpiecznikowy+wkladka/wylacznik nN); ranking deterministyczny. `ib_a`/`iz_prime_a` wymagane, `ik_max_ka` opcjonalny (brak = kryterium iii NIEROZSTRZYGALNE per kandydat). Koncowka jest read-only. | tests/api/test_nn_device_selection_api.py, tests/application/analyses/test_nn_device_selection.py | nadzor programu |
| `GET /api/cases/{case_id}/enm/swz` | v12xx | aktywny | 2026-08-13 | - | Werdykt SWZ (samoczynne wylaczenie zasilania, IEC 60364-4-41) per obwod (karta P0.6, G-06): 3-stanowy (spelnia/nie spelnia/nierozstrzygalne) + dowod liczbowy Ik1_min (scenariusz MIN, R skorygowane temperaturowo) vs Ia aparatu vs t_wymagany z Tab. 41.1. Koncowka jest read-only. | tests/api/test_kd9_kontrakt_koncowek_bez_testu.py, tests/application/analyses/swz/test_service.py | nadzor programu |
