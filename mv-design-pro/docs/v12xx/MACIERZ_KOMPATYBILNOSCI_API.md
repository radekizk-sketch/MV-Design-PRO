...
| `GET /api/analysis-runs/{run_id}/overlay` | v12xx | aktywny | 2026-08-14 | - | Dane nakladki SLD dla wyniku; oddaje takze `result_status` (NONE/FRESH/OUTDATED) z porownania odcisku modelu biegu z modelem biezacym oraz kod i zdanie przyczyny (pola addytywne 2026-08-14). | overlay contract tests + tests/api/test_protection_overlay_swiezosc.py | Architekt SLD |
...
| `GET /api/insights/n-1-contingency/scope` | v12xx | aktywny | 2026-08-14 | - | Zapowiedz zakresu enumeracji N-1 (karta EKRAN-N1, D8): lista kwalifikowanych elementow do wyboru zakresu + koszt biegu wyrazony liczba kontyngencji i liczba biegow solvera. Czyta wylacznie migawke (zero biegow solvera). Kosztu NIE wyraza czasem — kontrakt nie niesie pomiaru z biezacego modelu. | kontyngencje N-1 API tests | Architekt jakosci |
...
| `GET /api/projects/{project_id}/sld/{diagram_id}/protection-overlay` | v12xx | aktywny | 2026-08-14 | - | Nakladka SLD wynikow zabezpieczen (read-only, bez mutacji modelu i diagramu); `result_status` NONE/FRESH/OUTDATED z porownania odciskow modelu plus kod i zdanie przyczyny. ZMIANA 2026-08-14: bieg istniejacy, lecz niezakonczony albo bez zapisanego wyniku oddaje 200 ze statusem NONE i pusta lista elementow zamiast 400; 404 zostaje dla biegu nieistniejacego, 400 dla biegu z innego projektu. | tests/api/test_protection_overlay_swiezosc.py + tests/api/test_kd9_kontrakt_koncowek_bez_testu.py | nadzor programu |
...
| `GET /api/projects/{project_id}/sld/{diagram_id}/overlay` | v12xx | aktywny | 2026-08-14 | - | Nakladka wynikow na schemat jednokreskowy dla wskazanego biegu; odrzuca biegi legacy; `result_status` NONE/FRESH/OUTDATED z porownania odciskow modelu (wczesniej stale `VALID`) plus kod i zdanie przyczyny. | tests/test_production_canonical_only_api.py + tests/api/test_route_prefix_canon.py + tests/api/test_protection_overlay_swiezosc.py | nadzor programu |
