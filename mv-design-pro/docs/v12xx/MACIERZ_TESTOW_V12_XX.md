...
| V12-TST-018 | E2E lifecycle | Wykrywanie przegladarki i backend realny Playwright | `playwright-env.mjs` wykrywa cache Playwright Chromium, ma limit `PLAYWRIGHT_CHROMIUM_VERSION_TIMEOUT_MS`, `playwright-setup.mjs` uzywa `playwright.cmd` na Windows, `playwright.config.ts` startuje backend przez `cwd: backendCwd`, a krok `.github/workflows/frontend-e2e-smoke.yml` ma timeout zadania (`verify_v12_5.py` skasowany karta SUITA-BEZ-WYWOLANIA 2026-08-12 — nigdy nie stal w zadnym workflow CI). |
