...
| D-04 | **Pełny model ZIP + P(f)/Q(f) we WSZYSTKICH solverach** (NR, GS, FD) + pełne wpięcie ENM→katalog→solver (7+ builderów PQSpec) + **scalenie dwóch ścieżek NR** (ZASADA NR 3) | K-29, §8C.5 | **WDROŻONE** — ZIP+P(f) we wszystkich 3 solverach (reduce-to-NR bajt-identyczny, parytet NR/GS/FD), wpięcie ENM→katalog→solver kompletne, dwie ścieżki NR scalone w jedną (jeden solve + jeden Jakobian), ADR-011 pełny zakres (dowód niżej) | — |
...
**Spełnione (dowód w kodzie):** K-02, K-03, K-05, K-06, K-08 (sanity-bounds **12/12 analiz V12.6** + Ik'' wpięty w ścieżkę wyników SC — D-14/D-14b/D-14c, z testami), K-09, K-10, K-11, K-12, K-16, K-18, K-22, K-23, K-24 (rdzeń), K-28 (większość), K-29 (ZIP+P(f) we wszystkich solverach + QSTS — D-04).
