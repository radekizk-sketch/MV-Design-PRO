...
> **§2 SC — STAN ZMIENIONY (twardy przystanek B-01, czeka na decyzję):** `origin/main` przeszło do `0b87b0e` i ZAWIERA teraz zmianę trace SC z e6f2877 (r_ohm/x_ohm/z_equiv_abs_ohm). Revert §2 (`620595d`, słuszny gdy main tego nie miał) jest więc teraz ROZBIEŻNOŚCIĄ wobec main → `solver_boundary_guard` czerwony. Próba wyrównania (un-revert) **słusznie zablokowana** przez klasyfikator B-01 (jednostronna edycja zamrożonego rdzenia) — cofnąłem ją, SC pozostaje w stanie reverta §2. Decyzja właściciela wymagana: (a) autoryzuj wyrównanie do main (przywróć trace, guard zielony), (b) revert także w main (zmiana po stronie main), (c) zostaw revert + guard czerwony do scalenia.
