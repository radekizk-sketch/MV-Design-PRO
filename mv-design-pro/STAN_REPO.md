...
> - **Granica numeryczna (uczciwie):** przy bardzo stromym nachyleniu Q(U) w pu systemowym (źródło mocą ~ baza systemu) GS (punkt stały) i FD (stałe B'') mogą nie zbiec, podczas gdy NR (pełny Jakobian) zbiega. To **wrodzona własność metod** GS/FD (brak sprzężenia pochodnej volt-var), nie pominięcie — charakterystyka jest policzona we wszystkich 3, a NR jest solverem odpornym. Dla typowych mocy falowników (niskie nachylenie w pu systemowym) parytet zachowany. Opcjonalne rozszerzenie zasięgu GS/FD (under-relaxation) — do decyzji właściciela.
...
> - **§2 B-01 — diagnoza SC (`solver_boundary_guard`, twardy przystanek, czeka na decyzję właściciela):** guard flaguje `short_circuit_iec60909.py` zmieniony w e6f2877 (V12.6 SLD). Diagnoza (read-only): zmiana to **5 linii NIE-fizyki** w `_build_white_box_trace` — dodaje `r_ohm`/`x_ohm`/`z_equiv_abs_ohm` (re-projekcje już policzonego `z_equiv`) do kroku trace „Zk". **Zero zmiany wyników**: 158 testów SC zielonych, golden `proof.json` bajt-stabilne; `solver_diff_guard` (hash) jest **ZIELONY** dla SC (hash zgodny z odświeżoną bazą). Czerwony jest tylko `solver_boundary_guard` (oparty na `git diff origin/main...HEAD`, nie odróżnia trace od fizyki). Rekomendacja (NIE wykonana, B-01): (c) autoryzowany wyjątek w rejestrze / korekta zakresu guardu — rdzeń fizyki nietknięty; albo trywialny revert 5 linii (R/X/|Z| wyprowadzalne z `z_equiv_ohm`). NIE edytuję rdzenia SC jednostronnie.
