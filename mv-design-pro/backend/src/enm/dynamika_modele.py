...
class RegulatorNapiecia(BaseModel):
    ...
    ta_s: float = Field(gt=0.0, le=5.0, description="Stała czasowa AVR (s).")
    tb_s: float = Field(ge=0.0, le=20.0, description="Stała czasowa opóźnienia (s).")
    tc_s: float = Field(ge=0.0, le=20.0, description="Stała czasowa wyprzedzenia (s).")
    efd_min_pu: float = Field(le=0.0, ge=-10.0, description="Dolny limit napięcia wzbudzenia (pu).")
    efd_max_pu: float = Field(gt=0.0, le=10.0, description="Górny limit napięcia wzbudzenia (pu).")

    @model_validator(mode="after")
    def _limity_spojne(self) -> RegulatorNapiecia:
        if self.efd_min_pu >= self.efd_max_pu:
            raise ValueError(
                f"RegulatorNapiecia: efd_min_pu ({self.efd_min_pu}) musi być mniejsze "
                f"niż efd_max_pu ({self.efd_max_pu})."
            )
        return self
...
class RegulatorObrotow(BaseModel):
    ...
    r_pu: float = Field(gt=0.0, le=0.15, description="Statyzm regulatora obrotów (pu).")
    t1_s: float = Field(ge=0.0, le=20.0, description="Stała czasowa 1 (s).")
    t2_s: float = Field(ge=0.0, le=20.0, description="Stała czasowa 2 (s).")
    t3_s: float = Field(ge=0.0, le=20.0, description="Stała czasowa 3 (s).")
    p_max_pu: float = Field(gt=0.0, le=1.5, description="Górny limit mocy turbiny (pu).")
    ...
    def _limity_spojne(self) -> RegulatorObrotow:
        if self.p_min_pu >= self.p_max_pu:
            raise ValueError(
                f"RegulatorObrotow: p_min_pu ({self.p_min_pu}) musi być mniejsze "
                f"niż p_max_pu ({self.p_max_pu})."
            )
        return self
...
class StabilizatorSystemowy(BaseModel):
    ...
    tw_s: float = Field(gt=0.0, le=30.0, description="Stała czasowa wash-out (s).")
    t1_s: float = Field(ge=0.0, le=5.0, description="Stała czasowa wyprzedzenia 1 (s).")
    t2_s: float = Field(ge=0.0, le=5.0, description="Stała czasowa opóźnienia 1 (s).")
    t3_s: float = Field(ge=0.0, le=5.0, description="Stała czasowa wyprzedzenia 2 (s).")
    t4_s: float = Field(ge=0.0, le=5.0, description="Stała czasowa opóźnienia 2 (s).")
    limit_min_pu: float = Field(le=0.0, ge=-0.5, description="Dolny limit wyjścia PSS (pu).")
    limit_max_pu: float = Field(gt=0.0, le=0.5, description="Górny limit wyjścia PSS (pu).")

    @model_validator(mode="after")
    def _limity_spojne(self) -> StabilizatorSystemowy:
        if self.limit_min_pu >= self.limit_max_pu:
            raise ValueError(
                f"StabilizatorSystemowy: limit_min_pu ({self.limit_min_pu}) musi być "
                f"mniejsze niż limit_max_pu ({self.limit_max_pu})."
            )
        return self
...
class MaszynaSynchroniczna(BaseModel):
    ...
    h_s: float = Field(gt=0.0, le=15.0, description="Stała inercji H (s).")
    d_pu: float = Field(ge=0.0, le=20.0, description="Współczynnik tłumienia D (pu).")
    ...
    xd_prim_pu: float = Field(gt=0.0, le=0.6, description="Reaktancja przejściowa X'd (pu).")
    xq_prim_pu: float = Field(gt=0.0, le=0.8, description="Reaktancja przejściowa X'q (pu).")
    xd_bis_pu: float = Field(gt=0.0, le=0.35, description="Reaktancja podprzejściowa X''d (pu).")
    xq_bis_pu: float = Field(gt=0.0, le=0.4, description="Reaktancja podprzejściowa X''q (pu).")
    td0_prim_s: float = Field(gt=0.0, le=15.0, description="Stała czasowa T'd0 (s).")
    tq0_prim_s: float = Field(gt=0.0, le=2.0, description="Stała czasowa T'q0 (s).")
    td0_bis_s: float = Field(gt=0.0, le=0.2, description="Stała czasowa T''d0 (s).")
    tq0_bis_s: float = Field(gt=0.0, le=0.3, description="Stała czasowa T''q0 (s).")
...
class PrzeksztaltnikGFL(BaseModel):
    ...
    s_n_mva: float = Field(gt=0.0, le=500.0, description="Moc znamionowa przekształtnika (MVA).")
    i_max_pu: float = Field(ge=1.0, le=3.0, description="Maksymalny prąd przekształtnika (pu).")
    ...
    pll_ki: float = Field(gt=0.0, le=50000.0, description="Wzmocnienie całkujące PLL.")
    reg_pradu_kp: float = Field(
        gt=0.0, le=100.0, description="Wzmocnienie proporcjonalne regulatora prądu."
    )
    reg_pradu_ki: float = Field(
        gt=0.0, le=100000.0, description="Wzmocnienie całkujące regulatora prądu."
    )
    k_frt: float = Field(
        ge=0.0, le=10.0, description="Wzmocnienie prądu biernego przy zapadzie (FRT)."
    )
    prog_frt_pu: float = Field(ge=0.0, le=1.0, description="Próg napięciowy zadziałania FRT (pu).")
    tp_s: float = Field(gt=0.0, le=2.0, description="Stała czasowa filtru mocy czynnej (s).")
    tiq_s: float = Field(gt=0.0, le=2.0, description="Stała czasowa regulatora Iq (s).")
    ...
    p_odbudowa_opoznienie_s: float = Field(
        ge=0.0, le=5.0, description="Opóźnienie startu odbudowy P (s)."
    )
    droop_p_f_pu: float = Field(ge=0.0, le=0.2, description="Droop P/f (pu).")
    martwa_strefa_f_hz: float = Field(
        ge=0.0, le=1.0, description="Martwa strefa częstotliwości (Hz)."
    )
    droop_q_u_pu: float = Field(ge=0.0, le=0.2, description="Droop Q/U (pu).")
    martwa_strefa_u_pu: float = Field(ge=0.0, le=0.2, description="Martwa strefa napięcia (pu).")
    u_min_ciagle_pu: float = Field(ge=0.0, le=1.0, description="Dolna granica pracy ciągłej (pu).")
    u_max_ciagle_pu: float = Field(ge=1.0, le=1.5, description="Górna granica pracy ciągłej (pu).")

    @model_validator(mode="after")
    def _granice_napiecia_spojne(self) -> PrzeksztaltnikGFL:
        if self.u_min_ciagle_pu >= self.u_max_ciagle_pu:
            raise ValueError(
                "PrzeksztaltnikGFL: u_min_ciagle_pu musi być mniejsze niż u_max_ciagle_pu "
...
class PrzeksztaltnikGFM(BaseModel):
    ...
    h_wirtualne_s: float = Field(ge=0.0, le=20.0, description="Wirtualna stała inercji (s).")
    d_wirtualne_pu: float = Field(ge=0.0, le=100.0, description="Wirtualne tłumienie (pu).")
    r_wirtualne_pu: float = Field(
        ge=0.0, le=1.0, description="Wirtualna rezystancja wyjściowa (pu)."
    )
    x_wirtualne_pu: float = Field(
        ge=0.0, le=1.0, description="Wirtualna reaktancja wyjściowa (pu)."
    )
    i_max_pu: float = Field(ge=1.0, le=3.0, description="Maksymalny prąd przekształtnika (pu).")
...
class Magazyn(BaseModel):
    ...
    e_n_kwh: float = Field(gt=0.0, le=2_000_000.0, description="Pojemność znamionowa (kWh).")
    p_ladowania_max_kw: float = Field(gt=0.0, le=500_000.0, description="Maks. moc ładowania (kW).")
    p_rozladowania_max_kw: float = Field(
        gt=0.0, le=500_000.0, description="Maks. moc rozładowania (kW)."
    )
    sprawnosc_ladowania: float = Field(gt=0.0, le=1.0, description="Sprawność ładowania (pu).")
    sprawnosc_rozladowania: float = Field(
        gt=0.0, le=1.0, description="Sprawność rozładowania (pu)."
    )
    soc_min: float = Field(ge=0.0, le=1.0, description="Dolna granica SOC (pu).")
    soc_max: float = Field(ge=0.0, le=1.0, description="Górna granica SOC (pu).")
    soc_poczatkowy: float = Field(ge=0.0, le=1.0, description="Początkowy SOC (pu).")
    regulacja_f: RegulacjaCzestotliwosciMagazynu | None = None
    przeksztaltnik: PrzeksztaltnikGFL | PrzeksztaltnikGFM = Field(
        discriminator="rodzina", description="Przekształtnik magazynu (GFL albo GFM)."
    )
    ...
    def _soc_i_moc_spojne(self) -> Magazyn:
        if self.soc_min >= self.soc_max:
            raise ValueError(
                f"Magazyn: soc_min ({self.soc_min}) musi być mniejsze niż soc_max ({self.soc_max})."
            )
        if not (self.soc_min <= self.soc_poczatkowy <= self.soc_max):
            raise ValueError(
                f"Magazyn: soc_poczatkowy ({self.soc_poczatkowy}) musi leżeć w przedziale "
        ...
        if self.p_rozladowania_max_kw > baza_kw:
            raise ValueError(
                ...
                f"bazę mocy przekształtnika s_n_mva*1000 ({baza_kw})."
            )
...
class Crowbar(BaseModel):
    ...
    prog_pradu_pu: float = Field(
        gt=1.0, le=5.0, description="Próg prądu wirnika załączający crowbar (pu)."
    )
    czas_zwloki_s: float = Field(ge=0.0, le=1.0, description="Zwłoka załączenia crowbar (s).")
    czas_trwania_s: float = Field(
        gt=0.0, le=5.0, description="Czas trwania załączenia crowbar (s)."
    )
...
class TurbinaWiatrowa(BaseModel):
    ...
    h_calkowite_s: float = Field(gt=0.0, le=15.0, description="Całkowita stała inercji układu (s).")
    sztywnosc_walu_pu: float = Field(gt=0.0, le=500.0, description="Sztywność wału (pu).")
    tlumienie_walu_pu: float = Field(ge=0.0, le=10.0, description="Tłumienie wału (pu).")
    poslizg_ustalony_pu: float = Field(ge=-0.1, le=0.1, description="Poślizg ustalony (pu).")
    crowbar: Crowbar | None = None
    pitch_tempo_deg_s: float = Field(
        gt=0.0, le=30.0, description="Tempo zmiany kąta pitch (deg/s)."
    )
    pitch_min_deg: float = Field(ge=-10.0, le=10.0, description="Minimalny kąt pitch (deg).")
    pitch_max_deg: float = Field(gt=0.0, le=90.0, description="Maksymalny kąt pitch (deg).")
    ...
    def _spojnosc_typu(self) -> TurbinaWiatrowa:
        if self.pitch_min_deg >= self.pitch_max_deg:
            raise ValueError(
                f"TurbinaWiatrowa: pitch_min_deg ({self.pitch_min_deg}) musi być mniejszy "
                f"niż pitch_max_deg ({self.pitch_max_deg})."
            )
        if self.rodzina == "wiatr_typ_3":
            pass  # crowbar opcjonalny (nie kazda instalacja DFIG go ma)
        elif self.crowbar is not None:
            raise ValueError(
                f"TurbinaWiatrowa: crowbar dotyczy wyłącznie typ_3 (otrzymano {self.rodzina})."
            )
        if self.rodzina in ("wiatr_typ_3", "wiatr_typ_4"):
            if self.przeksztaltnik is None:
                raise ValueError(
                    f"TurbinaWiatrowa: {self.rodzina} wymaga bloku przekształtnik "
                    "(typ 3/4 ma przekształtnik częściowej/pełnej mocy)."
                )
        elif self.przeksztaltnik is not None:
            raise ValueError(
                f"TurbinaWiatrowa: przekształtnik dotyczy wyłącznie typ_3/typ_4 "
