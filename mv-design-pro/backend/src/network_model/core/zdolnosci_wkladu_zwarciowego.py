...
DECYZJA WŁAŚCICIELA, WIĄŻĄCA (2026-09-16, karta S-2 AUTORYTET): domyślka
...
DLACZEGO OSOBNY MODUŁ, A NIE ``if`` W JEDNYM MIEJSCU. Gdyby naprawa polegała na
dopisaniu warunku w jednym punkcie sprawdzania kwalifikowalności, każdy kolejny
konsument (dobór aparatu, pakiet dowodowy, koordynacja zabezpieczeń) musiałby
pamiętać o powtórzeniu tego warunku — a pierwszy, który zapomni, otwiera tę samą
dziurę. Lista zdolności jest tu RAZ i jest maszynowo czytelna (przypięta testem
zamknięcia — `test_kazda_zdolnosc_ma_jawna_klasyfikacje`).
...
class ZdolnoscMiarodajna(StrEnum):
    ...
    Kilka z nich nie ma dziś własnego typu analizy w ``SolverAnalysisType``
    (dobór aparatury, dowód wytrzymałości zwarciowej) — są tu mimo to, bo
...
#: Kod blokady — wynik nie niesie żadnego śladu pochodzenia (historyczny,
#: zdeserializowany, zbudowany ręcznie w kodzie wywołującego).
KOD_BLOKADY_WYNIK_BEZ_SLADU = "SI-112"
#: Kod blokady — wielkości zwarciowe podane wprost w żądaniu, bez modelu, z
#: którego by wynikały.
KOD_BLOKADY_WYNIK_Z_PAYLOADU = "SI-113"
#: Kod blokady — czynniki poprawne, ale iloczyn ``k_sc · I_n`` wychodzi poza
#: dziedzinę liczb skończonych. Osobny kod, bo osobna naprawa: projektant ma
...
#: ujemny albo nieskończony, więc wkładu NIE DA SIĘ policzyć. Osobny kod, bo
#: osobna naprawa: projektant ma uzupełnić daną tabliczkową źródła, a nie
#: współczynnik.
KOD_BLOKADY_PRAD_ZNAMIONOWY_NIEPOPRAWNY = "SI-115"
#: Kod blokady — znacznik proweniencji spoza ZAMKNIĘTEJ listy rozpoznawanych
