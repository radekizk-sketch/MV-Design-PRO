...
DLACZEGO TEN MODUŁ ISTNIEJE. Granica autorytetu wyniku zwarciowego (karta S-2,
`network_model.core.autorytet_wyniku_zwarciowego`) wiąże się z proweniencją
MODELU — pyta, czy źródła falownikowe mają zadeklarowane ``k_sc``. Nie pyta o
rzecz najważniejszą — czy LICZBY, których konsument zamierza użyć, pochodzą z
TEGO modelu.

OBEJŚCIE, KTÓRE TO ZAMYKA (nazwane w `docs/plan/
SYNTEZA_DOMKNIECIA_PRODUKTU_2026-09.md` §0 p. 5, zmierzone na HEAD `c307e95f`)::
    ...
    POST /api/equipment-proof/pack
      ...
      required_fault_results.ikss_ka = 12,5  -> HTTP 200, pakiet dowodowy
      required_fault_results.ikss_ka = 999,0 -> HTTP 200, pakiet dowodowy
...
implementacji, która go NIE policzyła.
"""
...
#: każdym dodaniu pola metadanych i przestałby cokolwiek znaczyć. Nazwy pól
#: 1:1 z wierszem surowego wyniku solvera (`raw_result["results"][i]`,
#: `network_model/solvers/short_circuit_iec60909.py::ShortCircuitResult.to_dict`,
#: skonsumowane przez `enm/canonical_analysis.py::build_short_circuit_results`).
...
def wszystkie_pola_obecne(zapisy: Iterable[Mapping[str, Any]]) -> bool:
