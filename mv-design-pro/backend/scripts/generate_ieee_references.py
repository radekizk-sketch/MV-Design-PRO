...
pandapower is executed for the IEEE cross-validation. It produces ONE committed
artifact per case, which does NOT depend on pandapower at runtime/CI:

  ``tests/golden/parytet_benchmarkow/expected/ieee_<n>bus.json``
     The independent reference VALUES (per-bus |V| and angle from pandapower's
     Newton-Raphson power flow) plus full provenance.

  (Karta K2, 2026-09-09: do tej daty skrypt pisał DRUGI artefakt — statyczny
  słownikowy builder ``src/application/reference_networks/builders/ieee_<n>bus.py``,
  wejście dawnego dialektu benchmarków kasowanego tą kartą. Ten builder miał
  ZERO konsumentów po kasacji — jedyny odbiorca był w skasowanym pakiecie.
  Tor produkcyjny/testowy dla case9/14/39 to dziś `tests/golden/enm_builders/
  ieee_<n>bus.py` — moduł EnergyNetworkModel pisany ręcznie, NIE generowany
  tym skryptem — patrz jego docstring. Emisja słownikowego buildera i katalog
  docelowy pod usuniętym `src/application/reference_networks/` zostały zdjęte
  z tego skryptu, żeby nie odtwarzać skasowanej ścieżki.)
...
PROVENANCE / NO-TAUTOLOGY:
  ...
  reference values come from running pandapower's ``runpp``. The validation
  tests (``tests/golden/parytet_benchmarkow/test_wyrocznia_a_expected_json.py::
  test_ieee_9bus_pf`` / ``test_ieee_14bus_pf_zgodny_z_wyrocznia_a`` /
  ``test_ieee_39bus_pf_zgodny_z_wyrocznia_a``, plus
  ``tests/golden/parytet_benchmarkow/kontrola_v126.py``) load the committed
  JSON, run OUR Newton-Raphson (via ``tests/golden/enm_builders/ieee_<n>bus.py``),
  and compare — so agreement is between two INDEPENDENT implementations.
...
TESTS = ROOT / "tests"
EXPECTED_DIR = TESTS / "golden" / "parytet_benchmarkow" / "expected"
...
def _write_expected(case_n: int, enm: dict[str, Any], net: Any, pp_version: str) -> Path:
    ...
    payload = {
        ...
        "source_note": (
            ...
            f"(algorithm='nr', tolerance_mva=1e-10). The internal translation used to "
            f"drive pandapower was verified structurally identical to case{case_n} "
            f"(fidelity asserted at generation time, not committed). Regenerate with: "
            f"pip install pandapower=={pp_version}; "
...
def main() -> int:
    ...
    parser.add_argument("--confirm", action="store_true", help="Write expected JSON")
    parser.add_argument(
        "--cases",
        type=int,
        nargs="+",
