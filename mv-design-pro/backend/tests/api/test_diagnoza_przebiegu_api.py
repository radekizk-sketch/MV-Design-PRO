...
# ---------------------------------------------------------------------------
# W1 (2026-09-09): trzecia trasa modułu — `GET /api/cases/{id}/enm/diff` —
# skasowana razem z `diagnostics/diff.py` i migawkami legacy (`uow.snapshots`):
# porównywała rewizje z tabel `network_*`, których żaden tor użytkownika nie
# zapisywał (0 konsumentów produkcyjnych — frontend nie wołał tej końcówki).
