...
def _t12_zaakceptowany_przez_profil(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    DLACZEGO PODMIANA REJESTRU, A NIE REALNA ŚCIEŻKA. Od karty AB-1a (R-6) T12 jest
    twierdzeniem o zachowaniu w czasie (DYNAMIC_PERFORMANCE) i jest niedopuszczalny
    dowodowo; dla klasy A bez certyfikatu to jedyny test wymagany, a żadna inna
    realna konfiguracja modułu nie ma wymaganego testu dopuszczalnego z werdyktem
    ``fail``. Własność „werdykt negatywny NIE blokuje dokumentu, gdy dowód jest
    dopuszczalny" jest cechą bramki certyfikatu — testujemy ją na bramce z jawnie
    nazwaną podmianą jednej pozycji rejestru, a realną ścieżkę (T12 blokuje z
    nazwą) pinuje `test_t12_z_samej_deklaracji_blokuje_certyfikat_z_nazwa`.
    """
    from solver_input import dowod_ncrfg
    from solver_input.provenance import ClaimKind

    tabela = dict(dowod_ncrfg.TEST_ZDOLNOSC)
    tabela["T12"] = (
        "ncrfg_ptpiree.declared_configuration",
        ClaimKind.DECLARED_CONFIGURATION,
    )
    monkeypatch.setattr(dowod_ncrfg, "TEST_ZDOLNOSC", tabela)


def test_certyfikat_negatywny_powstaje_z_werdyktem_niezgodnym(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # INTENCJA (bez zmian): werdykt negatywny przy dopuszczalnym dowodzie NIE
    # blokuje — dokument stwierdza stan. Klasa A BEZ certyfikatu PTPiREE: T12
    # staje się WYMAGANY; rampa dużo wolniejsza niż referencyjna daje `fail`.
    # Dopuszczalność dowodu T12 — patrz `_t12_zaakceptowany_przez_profil`.
...
def test_t12_z_samej_deklaracji_blokuje_certyfikat_z_nazwa() -> None:
    """Karta AB-1a R-6 (pin „PPM typu A z samą deklaracją T12 nie jest
    raportowalny"): realna ścieżka — certyfikat NIE powstaje, a brak NAZYWA test
    T12 i stan dowodowy (deklaracja), niezależnie od werdyktu testu."""
    for rampa in (0.1, 10.0):
        run_result = _run_result(
            _module(
                certificate_status="unknown",
                stop_generation_enabled=True,
                ramp_rate_pct_per_min=rampa,
            )
        )
...
def test_thd_modulu_blokuje_dokument_z_nazwanym_t20() -> None:
    """Karta AB-1a R-6: podane THD czyni T20 wymaganym, a T20 jest od tej karty
    niedopuszczalny dowodowo — brak nazywa T20 i jego stan (deklaracja)."""
    run_result = _run_result(_module(harmonic_thdu_percent=3))
    braki = zbierz_braki(run_result)
    assert any("T20" in b and "zdolność: T20:DECLARATION" in b for b in braki), braki
