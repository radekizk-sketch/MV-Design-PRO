...
``pole_wymagane``/``pole_z_aliasem`` byly uzywane w calej rodzinie budowniczych
wejscia solvera z fikstur referencyjnych starego dialektu (computation.py,
frozen_solver_input.py, pandapower_bridge.py — skasowane karta K2, 2026-09-09)
— brak pola fikstury to blad AUTORSTWA fikstury (nie brakujacy WYNIK), wiec
zglaszamy blad z nazwa pola zamiast cichego 0.0. Jedynym zyjacym konsumentem
po K2 jest `sld_network_model.py` (ten sam katalog); modul przeniesiony z
`application/reference_networks/wymagane.py` bez zmiany tresci.
"""
...
from tests.golden.wymagane import (
