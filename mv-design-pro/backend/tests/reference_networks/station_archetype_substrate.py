...
READ-ONLY w.r.t. the frozen core (B-01): this only constructs the
...
Relocated from ``application/reference_networks/`` in card K2 (2026-09-09):
the old dialect this module lived alongside (``builders/``, ``computation.py``,
``library.py``) is gone, but THIS module's own construction mechanism (a
hand-built ``NetworkGraph``/``PowerFlowInput``, bypassing ENM) is UNCHANGED —
routing it through ``enm/kompilator_grafu.py`` would need new catalog records
for every hardcoded archetype parameter (lines/transformers/inverters/machines
across ~20 topologies) plus exhaustive companion byte-parity verification, out
of scope for K2 (flagged in the K2 report as a measured follow-up, not a silent
skip). This is now purely a proof-data generator for the isolated
``station-rozdzielnia`` v2 harness (``frontend/station-rozdzielnia-harness.html``,
``e2e/station-rozdzielnia-screenshot.spec.ts`` — NOT part of the routed app),
so it lives under ``tests/`` rather than ``application/``.

Run ``python -m
tests.reference_networks.station_archetype_substrate --write`` to
...
def _companions_dir() -> str:
    ...
    # K2 (2026-09-09): relocated from backend/src/application/reference_networks/
    # (4 hops to repo root) to backend/tests/reference_networks/ (3 hops) — one
    # level shallower, so the walk-up count changed too.
