...
def test_overcurrent_skeleton_missing_sc_field_raises_not_zero(uow_factory) -> None:
    """FAB-E (E1): brak pola wymaganego przez zamrożony ShortCircuitResult w
    zapisanym payloadzie (np. ikss_a) to uszkodzony artefakt -> odmowa z nazwą
    pola, nie fikcyjny prąd zwarciowy 0.0 A wpięty w nastawy zabezpieczeń."""
    sc_result = _build_short_circuit_result()
    sc_run_id = "sc-run-incomplete"
    sc_envelope = to_run_envelope(
        sc_result,
        run_id=sc_run_id,
        case_id="case-1",
    ...
    sc_index_entry = index_run(
        ...
        base_snapshot_id="snapshot-1",
        case_id="case-1",
        meta={"short_circuit_result": payload},
    )
    with uow_factory() as uow:
        if uow.analysis_runs_index.get(sc_index_entry.run_id) is None:
            uow.analysis_runs_index.add(sc_index_entry)
