"""Trwały test diagnostyczny V12K-027 (WYMÓG WŁAŚCICIELA 1 + 3, karta §2a).
...
def _minimal_net(*, load_q_mvar: float, cap_mvar: float | None = None) -> EnergyNetworkModel:
    ...
    return EnergyNetworkModel(
        header=ENMHeader(name="Dowod V12K-027"),
        buses=[
...
def _pomiar(load_q_mvar: float, cap_mvar: float | None) -> dict[str, float]:
    ...
    return {
        ...
        "q_from": float(
            next(b for b in rv["branch_results"] if abs(float(b["q_to_mvar"])) > 0.1)["q_from_mvar"]
        ),
        "p_to": p_to,
        "q_to": q_to,
        "q_cap_eff": q_cap_eff,
...
def test_odniesienie_bez_kompensacji() -> None:
    m = _pomiar(0.5, None)
    assert m["v_load"] == pytest.approx(1.010460, abs=1e-6)
    assert m["q_to"] == pytest.approx(+0.500000, abs=1e-6)
    assert m["slack_q"] == pytest.approx(-0.490750, abs=1e-6)
    assert m["cosfi_przekroju"] == pytest.approx(0.894427, abs=1e-6)  # cosφ_D8 (przekrój)


def test_odniesienie_cap_05() -> None:
    m = _pomiar(0.5, 0.5)
    assert m["v_load"] == pytest.approx(1.014260, abs=1e-6)
    assert m["q_to"] == pytest.approx(+1.014362, abs=1e-5)  # anomalia shuntu: q_to ROŚNIE
    assert m["slack_q"] == pytest.approx(-0.999461, abs=1e-5)
    assert m["cosfi_przekroju"] == pytest.approx(0.702047, abs=1e-5)  # cosφ_D8 MALEJE (defekt)
