...
| `dynamic_lab/reinicjalizacja.py` | re-inicjalizacja po zmianie topologii: stany różniczkowe TRZYMANE (ciągłość strumienia, kąta i prędkości), algebra rozwiązana OD NOWA na nowej topologii, uzgodnienie wyłącznie wobec zadeklarowanych ograniczników; diagnostyka Δx, Δy, ‖f‖, ‖g‖, residuum KCL liczone NIEZALEŻNIE od solvera i residuum szyn sztywnych |
| `dynamic_lab/bazy_bess.py` | łańcuch baz magazynu urządzenie↔sieć: JEDNA droga od mocy znamionowej [MVA] przez bazę sieci, p.u., MW, MWh/s do dSOC/dt; przelicznik godziny w jednym miejscu; audyt spójności kroków i test niezmienniczości trajektorii fizycznej wobec zmiany bazy mocy |
...
| `dynamic_lab/pomiar_zgodnosci.py` | POMIAR jako obiekt niewypisywalny ręcznie: wartość metryki powstaje wyłącznie w `zmierz_zgodnosc` (żeton tożsamościowy), jest PRZELICZALNA z własnych próbek i ZWIĄZANA odciskiem scenariusza — zamyka drogę „kompletny dowód bez uruchomienia symulacji" |
| `dynamic_lab/frt_z_biegu.py` | ocena FRT KONSUMUJE ślad `WynikDynamiczny`: kanały zadeklarowane per kryterium (klucz, przestrzeń, jednostka, element), powody nieorzekalności jako zamknięty słownik maszynowy, własności biegu (zbieżność, kompletność, kroki poniżej tolerancji) jako warunek orzekania, semantyka okna `[t_zakłócenia, +horyzont]` w jednym miejscu; stan przyłączenia pozostaje jawnie NIEODTWARZALNY ze śladu |
| `dynamic_lab/ryzyko_postaci_modelu.py` | INWENTARZ ograniczników i nasyceń z klasyfikacją ryzyka postaci modelu (postać wymuszona / alternatywa zmierzona / alternatywa niezmierzona / luka modelu); lista ZAMKNIĘTA i pilnowana skanem drzewa składni wszystkich modułów, każdy wpis wskazuje ISTNIEJĄCY test przypinający |
| `dynamic_lab/macierz_pokrycia.py` | MACIERZ POKRYCIA: wychodzi od ZDOLNOŚCI (co laboratorium twierdzi, że modeluje) i dla każdej liczy najmocniejszy dowód z rejestru; „brak danych" NIGDY nie daje potwierdzenia, a lista zdolności jest pilnowana skanem klas urządzeń |
| `dynamic_lab/porownanie_postaci.py` | UPRZĄŻ PORÓWNANIA POSTACI: dwa warianty tym samym zadaniem, różnica mierzona kanał po kanale na wspólnej osi czasu; uprząż NIE WYBIERA postaci kanonicznej (brak pola „zalecany" jest przypięty testem), a „postacie nierozróżnialne" znaczy bit w bit i jest meldowane jako BRAK DOWODU RÓŻNICY, nie jako równoważność |
