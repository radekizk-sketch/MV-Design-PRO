---
name: worker-rdzen
description: Implements a solver-core or FROZEN-adjacent card (dynamics engine, oracles, safety gates) — the worker role with raised effort. Implements one card of MV-DESIGN-PRO end-to-end inside the git worktree named in the card — edits, tests as a product of features, guards, fixtures, one local commit without push, and a written report. Use for every code change that must be verified.
